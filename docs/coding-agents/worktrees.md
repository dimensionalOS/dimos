(doc-coding-agents-worktrees-worktrees)=

# Worktrees

`bin/worktree` creates a fully-provisioned git worktree (uv venv + `uv sync --all-groups`, `.envrc` symlink, direnv allow) so `mypy`/`pytest`/hooks work immediately — handy for forking off parallel work.
...
Run commands in a worktree from outside it (direnv isn't auto-loaded in a non-interactive shell):
