(doc-coding-agents-code-quality-rules-code-quality-rules)=
...
(doc-coding-agents-code-quality-rules-architecture-and-separation-of-concerns)=
...
(doc-coding-agents-code-quality-rules-blueprints)=
...
- Do no work at import time: no subprocesses, viewers, model parsing, or network. In particular don't call `get_data(...)` (it blocks import until the download finishes) -- use [`LfsPath`][LfsPath] (resolved at access time) or build the config in `start`/`build`. Any process you start must be managed (shut down when not needed).
...
(doc-coding-agents-code-quality-rules-concurrency-and-thread-safety)=

## Concurrency and thread safety

- Boolean stop/started flags (`self._running`, `self._started`, `self._shutdown`, ...) aren't thread safe. Use a `threading.Event`.
- Any state touched by more than one thread needs a lock everywhere -- the read side too, not just writes. [`@rpc`][rpc] methods run on different threads, so their state needs a lock.
...
(doc-coding-agents-code-quality-rules-configuration)=
...
- Don't bake personal/hardware config into source or blueprints: IPs (`192.168.x.x`), interface names (`enp86s0`), default IPs in constructors. Use a [`GlobalConfig`][GlobalConfig] field with a sensible default (often `None`) and set it via `.env` or CLI.
...
(doc-coding-agents-code-quality-rules-constants-magic-numbers-and-centralizing-values)=
...
(doc-coding-agents-code-quality-rules-constructors-and-object-initialization)=
...
- Don't do work or start things in `__init__` (no autostart). The constructor sets the object up; it doesn't run it. Doing work there:
  ...
  - leaves it in an invalid, hard-to-clean-up state if it fails. Move the work to `start()` (or `build()`).

(doc-coding-agents-code-quality-rules-dead-commented-out-and-stale-code)=
...
(doc-coding-agents-code-quality-rules-documentation)=
...
(doc-coding-agents-code-quality-rules-duplication-and-reuse)=
...
(doc-coding-agents-code-quality-rules-error-handling)=
...
(doc-coding-agents-code-quality-rules-imports)=
...
(doc-coding-agents-code-quality-rules-inheritance-vs-composition)=
...
(doc-coding-agents-code-quality-rules-logging)=
...
(doc-coding-agents-code-quality-rules-naming)=
...
(doc-coding-agents-code-quality-rules-performance)=
...
(doc-coding-agents-code-quality-rules-repository-hygiene-and-data-files)=
...
(doc-coding-agents-code-quality-rules-resource-lifecycle-and-cleanup)=
...
- Transports and publishers ([`LCMTransport`][LCMTransport], [`pLCMTransport`][pLCMTransport], ...) are often created but never closed; add a `stop()`. But a transport shared between modules shouldn't be closed by one of them.
...
(doc-coding-agents-code-quality-rules-tests)=
...
(doc-coding-agents-code-quality-rules-type-hints-and-mypy)=
