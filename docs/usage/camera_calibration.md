...
2. Confirm `distortion_model` is `plumb_bob`. dimos `CameraInfo.from_yaml` reads this key (not `camera_model`); other values are only valid if every consumer in your pipeline agrees on the same model.

3. `frame_id`: the YAML written by `dimos cameracalibrate` does not embed `frame_id`. When you build a `CameraInfo` from the file (for example `CameraInfo.from_yaml(path, frame_id=...)`), the `frame_id` you pass must match the `Image.header.frame_id` used for that camera in your graph. The stock `Webcam` publishes color images with frame id `camera_optical` (or `{frame_id_prefix}/camera_optical` if `frame_id_prefix` is set). The no-robot desk blueprint (ticket T5 in the feature backlog) publishes TF so that optical frame is consistent with that naming; keep your loaded `CameraInfo` header aligned with whatever your desk stack actually publishes for the optical camera.
