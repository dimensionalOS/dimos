...
Pre-built Nix store paths for the native modules are published to the `dimensionalos` Cachix cache. Opt in locally to skip cold compiles when the cache has them:
