(doc-usage-native-modules-native-modules)=

# Native Modules

(doc-usage-native-modules-native-modules-1)=
...
(doc-usage-native-modules-config)=
...
(doc-usage-native-modules-excluding-fields)=
...
(doc-usage-native-modules-using-with-blueprints)=
...
(doc-usage-native-modules-logging)=
...
(doc-usage-native-modules-json-log-format)=
...
(doc-usage-native-modules-writing-the-c-side)=

## Writing the C++ side

A header-only helper is provided at [dimos/hardware/sensors/lidar/common/dimos_native_module.hpp](https://github.com/dimensionalOS/dimos/blob/main/dimos/hardware/sensors/lidar/common/dimos_native_module.hpp):
...
(doc-usage-native-modules-examples)=

## Examples

For language interop examples (subscribing to DimOS topics from C++, TypeScript, Lua), see [/examples/language-interop/](https://github.com/dimensionalOS/dimos/blob/main/examples/language-interop/README.md).

(doc-usage-native-modules-livox-mid-360-module)=

### Livox Mid-360 Module

The Livox Mid-360 LiDAR driver is a complete example at [dimos/hardware/sensors/lidar/livox/module.py](https://github.com/dimensionalOS/dimos/blob/main/dimos/hardware/sensors/lidar/livox/module.py):
...
(doc-usage-native-modules-auto-building)=

## Auto Building

If `build_command` is set in the module config, and the executable doesn't exist when `start()` is called, NativeModule runs the build command automatically. Build output is piped through structlog (stdout at `info`, stderr at `warning`).
...
(doc-usage-native-modules-faster-builds-via-the-cachix-substituter)=
