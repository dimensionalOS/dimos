(doc-usage-python-api-python-api)=
...
(doc-usage-python-api-local-mode)=
...
(doc-usage-python-api-rpc-calls)=

## RPC calls

Modules can define [`@rpc`][rpc] methods which you can call. Here's an example:
...
(doc-usage-python-api-peeking-streams)=

## Peeking streams

`peek_stream(name, timeout)` pulls the next message from any running module's stream. Useful for quick inspection without writing a subscriber:
...
(doc-usage-python-api-remote-mode)=
...
`DimOS.connect()` finds the daemon on the local LCM bus. DimOS supports one daemon per LCM bus; set `LCM_DEFAULT_URL` to put daemons on different buses or to connect across hosts.
...
Strings and registered Module classes take a name-based fast path. Other Module classes and [`Blueprint`][Blueprint] objects are pickled and unpickled on the daemon, so their module classes must be importable there and all kwargs must be picklable.

(doc-usage-python-api-limitations)=

## Limitations

- `stop()` on a connected instance closes the LCM connection but does not terminate the remote process. Use `dimos stop` for that.

(doc-usage-python-api-restarting-modules)=
...
(doc-usage-python-api-what-needs-a-daemon-restart)=

### What needs a daemon restart

Hot-restart (`app.restart(MyModule)`) reloads the module's source, so the body of `start()`, handlers, and [`@rpc`][rpc] methods all pick up changes. But the following require a full daemon restart (`dimos stop` then `dimos run ...`):
...
If you find yourself needing data from an existing module that isn't on its [`Out`][Out] streams, the canonical fix is to add an `Out[T]` to that module and restart the daemon — don't spin up a parallel connection to the underlying hardware.

(doc-usage-python-api-operational-gotchas)=
...
- [`load_blueprint`][load_blueprint] over LCM has a 120s RPC timeout. If it raises `TimeoutError` after that long, the module may still have been deployed and started — check the daemon log for the `Deployed module` entry before assuming failure.

[rpc]: #dimos.core.core.rpc
[Blueprint]: #dimos.core.coordination.blueprints.Blueprint
[Out]: #dimos.core.stream.Out
[load_blueprint]: #dimos.core.coordination.module_coordinator.ModuleCoordinator.load_blueprint
