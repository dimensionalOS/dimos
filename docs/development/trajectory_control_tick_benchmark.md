...
```bash
pytest -m slow -sv dimos/navigation/test_trajectory_control_tick_benchmark.py
```

Optional: print medians and implied headroom in Hz (still runs the same assertions):

```bash
DIMOS_TRAJECTORY_BENCH_VERBOSE=1 pytest -m slow -sv dimos/navigation/test_trajectory_control_tick_benchmark.py
```
