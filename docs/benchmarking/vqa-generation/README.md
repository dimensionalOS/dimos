...
For the exact generation stages, checks, tool contracts, and output files, see [Pipeline](/docs/benchmarking/vqa-generation/pipeline.md).
...
Frozen recording -> rectified RGB image + calibrated visible point cloud
        ...
        |
        +-- agentic: image question author -> frozen question
...
Rejected questions and private evidence remain in each frame's generation record. The evaluator does not load point clouds, calibration, tool traces, or rejection records.
...
The generation models are code-level defaults, not CLI settings. Constrained mode always uses the image author, then expands its object queries into the deterministic families.
...
Constrained families run predefined sequences over the private perception primitives. Agentic generation exposes those same primitives to the oracle, which chooses its own sequence after the public question is frozen.
...
```json
{"id":"go2-40-chair-height","image":"frame-000040/image.jpg","question":"How tall is the chair?","choices":["under 0.2 m","0.2-0.6 m","0.6-1.0 m","over 1.0 m"]}
```
