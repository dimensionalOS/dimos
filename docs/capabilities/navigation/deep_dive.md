...
myst:
  substitutions:
    all_layers: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/5-all.png
      ```
    global_costmap: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/3-globalcostmap.png
      ```
    global_map: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/2-globalmap.png
      ```
    go2_navigation_data_flow: |-
      ```{image} assets/go2nav_dataflow.svg
      ```
    lidar_frame: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/1-lidar.png
      ```
    live_go2_navigation_in_rerun: |-
      ...
      ```
    navigation_costmap_with_path: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/4-navcostmap.png
      ```
    patrol_path: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/patrol_path.png
      ```
    coverage: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/coverage.png
      ```
    frontier: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/frontier.png
      ```
    random: |-
      ```{image} https://raw.githubusercontent.com/dimensionalOS/dimos-docs-assets/main/capabilities/navigation/assets/random.png
      ```
    unitree_go2_blueprint_module_graph: |-
      ```{image} assets/go2_blueprint.svg
      ```
---

(doc-capabilities-navigation-deep-dive-go2-navigation-deep-dive)=

# Go2 Navigation Deep Dive

(doc-capabilities-navigation-deep-dive-go2-navigation-deep-dive-1)=
...
(doc-capabilities-navigation-deep-dive-pipeline-steps)=

## Pipeline Steps

(lidar-frame-go2connection)=

(doc-capabilities-navigation-deep-dive-1-lidar-frame-go2connection)=

### 1. LiDAR Frame

Implemented by
[GO2Connection](https://github.com/dimensionalOS/dimos/blob/main/dimos/robot/unitree/go2/connection.py).
...
(global-voxel-map-voxelgridmapper)=

(doc-capabilities-navigation-deep-dive-2-global-voxel-map-voxelgridmapper)=

### 2. Global Voxel Map

The [VoxelGridMapper](https://github.com/dimensionalOS/dimos/blob/main/dimos/mapping/voxels.py) maintains a sparse 3D occupancy grid using Open3D's `VoxelBlockGrid` backed by a hash map. Each voxel is a 5cm cube by default.
...
(global-costmap-costmapper)=

(doc-capabilities-navigation-deep-dive-3-global-costmap-costmapper)=

### 3. Global Costmap

The [CostMapper](https://github.com/dimensionalOS/dimos/blob/main/dimos/mapping/costmapper.py) converts the 3D voxel map into a 2D occupancy grid. The default algorithm (`height_cost`) maps rate of change of Z, with some smoothing.

Algorithm settings live in [occupancy.py](https://github.com/dimensionalOS/dimos/blob/main/dimos/mapping/pointclouds/occupancy.py) and can be configured per robot.

(configuration-1)=

(doc-capabilities-navigation-deep-dive-configuration-1)=
...
(navigation-costmap-replanningastarplanner)=

(doc-capabilities-navigation-deep-dive-4-navigation-costmap-replanningastarplanner)=

### 4. Navigation Costmap

Implemented by
[ReplanningAStarPlanner](https://github.com/dimensionalOS/dimos/blob/main/dimos/navigation/replanning_a_star/module.py).
...
(doc-capabilities-navigation-deep-dive-5-all-layers-combined)=
...
(doc-capabilities-navigation-deep-dive-frontier-exploration)=

## Frontier Exploration

The [WavefrontFrontierExplorer](https://github.com/dimensionalOS/dimos/blob/main/dimos/navigation/frontier_exploration/wavefront_frontier_goal_selector.py) drives autonomous exploration of unknown space. It scans the costmap for frontiers, the boundaries between mapped and unmapped cells, picks the best candidate with a wavefront BFS from the robot's position, and publishes it as a navigation goal. When a goal is reached (or fails), it selects the next frontier until the space is fully mapped. Like patrolling below, it is exposed as an agent skill: an LLM agent can call `begin_exploration` and `end_exploration`.

(doc-capabilities-navigation-deep-dive-patrolling)=
...
(doc-capabilities-navigation-deep-dive-how-it-works)=
...
(doc-capabilities-navigation-deep-dive-patrol-router-strategies)=
...
(doc-capabilities-navigation-deep-dive-safety)=
...
(doc-capabilities-navigation-deep-dive-router-comparison)=
...
(doc-capabilities-navigation-deep-dive-sample-patrol-trace-26-min)=

### Sample patrol trace (26 min)

{{ patrol_path }}

(doc-capabilities-navigation-deep-dive-blueprint-composition)=
