...
1. Record a walk-through with `unitree-go2`
...
Throughout this guide, `{DB_NAME}` is the path of your recording, for example `recordings/20260826-165100-unitree-go2/memory.mcap`. For `map_file`, pass the same stem and dimOS appends `.pc2.lcm` automatically.
...
```bash
dimos --record --robot-ip {YOUR_ROBOT_IP} run unitree-go2
```
...
This writes `recordings/<run-id>/memory.mcap` (e.g. `recordings/20260826-165100-unitree-go2/memory.mcap`, the same id as the log dir) under the checkout (`~/.local/state/dimos/recordings/` for an installed package) and records `lidar`, `odom`, and `color_image` plus the live TF tree. `--record-format sqlite` writes `memory.db` instead. The recorder stamps lidar frames with the latest odom pose so `dimos map global` can reconstruct poses later- see [`OdomRecorder`](/dimos/memory/module.py).
...
```bash
dimos mem summary {DB_NAME}
dimos map replay {DB_NAME} --duration 60
```
...
```bash
dimos map global {DB_NAME} --export --no-gui
...
exporting PGO twopass map to .../{DB_NAME}.pc2.lcm...
wrote .../{DB_NAME}.pc2.lcm
```
...
```
Relocalization module started: map_file='{DB_NAME}'  loaded_map.frame_id='map'
...
|------|--------|-------------|-------------|
| `{name}.db` | memory SQLite (`lidar`, `odom`, `color_image`, …) | `unitree-go2` | `dimos map *`, `--replay-db` |
