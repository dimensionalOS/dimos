...
Throughout this guide, `{DB_NAME}` is the path of your recording, for example `recordings/20260826-165100-unitree-go2/memory-12345.db`. For `map_file`, pass the same stem and dimOS appends `.pc2.lcm` automatically.
...
`--record` writes every published topic to `recordings/<run-id>/memory-<pid>.db` under the checkout (`~/.local/state/dimos/recordings/` for an installed package); one file per worker process, `<run-id>` matches the `logs/` directory. `--record-topics lidar,odom,color_image,tf` limits it. Poses are not stored per frame; run `dimos map pose-fill` to derive them from the recorded `tf`/`odom` before building a map.
