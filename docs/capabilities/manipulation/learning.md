...
One file per run, never overwritten. It holds three streams: `color_image`, `coordinator_joint_state`, and the episode start/save/discard markers. Ten to twenty good episodes is a reasonable first dataset.
...
`--policy.device=mps` on Apple silicon, `cpu` for a slow smoke test. ACT trains to usable on tens of episodes; the checkpoint you deploy lands at:
