...
| Galaxea A1Z | keyboard | ✓ | camera | hand-teach → policy |
| A-750 | keyboard | ✓ | — | — |
| OpenArm (bimanual) | keyboard | ✓ | — | — |

Nearly every workflow also runs without hardware — blueprints fall back to a mock arm or MuJoCo, so you can try planning, teleop, and VR collection before an arm is on your desk. The A1Z hand-teach flow is the exception: it needs the real arm.

Per-arm setup lives with the platform, not here: [A-750](/docs/platforms/arms/a750.md), [OpenArm](/docs/platforms/arms/openarm.md), and the A1Z README in `dimos/robot/manipulators/galaxea_a1z/`.
