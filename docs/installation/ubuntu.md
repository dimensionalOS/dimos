...
```sh skip
curl -fsSL https://raw.githubusercontent.com/dimensionalOS/dimos/main/scripts/install.sh | bash -s -- --mode dev --project-dir ./dimos --capabilities navigation,manipulation
cd dimos
...
Both modes pass CPU installation CI on Ubuntu 22.04/24.04 and x86_64/ARM64. The capability bundles do not include the specialized `scene` extra, which is unavailable on Linux ARM64. Jetson CUDA setup is not supported.
