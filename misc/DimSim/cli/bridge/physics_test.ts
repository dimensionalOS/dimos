...
import { MOTION_MODELS, ServerPhysics } from "./physics.ts";
...
Deno.test("holonomic motion applies forward and lateral velocity in the local frame", () => {
  const output = MOTION_MODELS.holonomic(
    { linX: 1, linY: 2, linZ: 0, angZ: 0 },
    Math.PI / 2,
    {
      gravity: -9.81,
      maxAltitude: 100,
  ...
  assertClose(output.dx, 0.5);
  assertClose(output.dz, -1);
});
