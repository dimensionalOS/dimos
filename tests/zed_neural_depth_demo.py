...
class ZEDLiveVisualizer:
    ...
    def update_stats(self, depth_map, frame_count):
        ...
        if len(valid_depth) > 0:
            stats = (f"Frame {frame_count} | Valid pixels: {len(valid_depth):,} | "
...
def main():
    ...
    parser.add_argument("--fps-limit", type=float, default=30.0,
                       help="Display FPS limit (default: 30.0)")
    parser.add_argument("--camera-fps", type=int, default=30,
                       help="Camera capture FPS (default: 30)")
    parser.add_argument("--depth-mode", type=str, default="NEURAL",
    ...
    try:
        ...
        logger.info(f"Initializing ZED camera with {args.depth_mode} depth processing at {args.camera_fps} FPS...")
        camera = ZEDCamera(
        ...
        with camera:
            ...
            if total_time > 0:
                ...
                logger.info(f"Final stats: {frame_count} frames in {total_time:.1f}s (avg {avg_fps:.1f} FPS)")
