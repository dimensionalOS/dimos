...
def visualize_multiple_frames(
    ...
    processed = 1
    progress_info = (
        f"processed {processed}/{num_frames} frames" if total_frames else f"frame {start_idx}"
    )
    print(
        f"Frame {start_idx}: {len(frames[0].pointcloud.points)} → {len(map_pcd.points)} points (after proper voxelization) - {progress_info}"
    )
    ...
    for i, frame in enumerate(frames[1:], start=1):
        ...
        processed = i + 1
        progress_info = (
            f"processed {processed}/{num_frames} frames" if total_frames else f"frame {frame_idx}"
        )
        ...
        print(
            f"Frame {frame_idx}: {len(frame.pointcloud.points)} → {len(new_pcd.points)} points → Map: {len(map_pcd.points)} points - {progress_info}"
        )
...
def main() -> None:
    ...
    try:
        ...
        for i in range(num_frames):
            ...
            current_num = i + 1
            print(
                f"Loaded frame {current_num}/{num_frames} (frame index {frame_idx}/{total_frames - 1})",
                end="\r",
            )
