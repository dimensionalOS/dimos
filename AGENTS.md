...
- **DDSTransport**: DDS pub/sub, imports cyclonedds when a `DDSTransport` is constructed; install with `uv sync --extra dds --inexact` (`dimos/protocol/pubsub/impl/ddspubsub.py`).
