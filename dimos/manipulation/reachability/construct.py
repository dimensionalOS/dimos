...
Sampling goes through MuJoCo directly rather than the planning
``WorldSpec``: it needs only a compiled model, so construction does not
depend on a particular planning backend being installed.
"""
...
def arm_spec(robot: str = "g1-left", params: MapParams | None = None) -> ConstructionSpec:
    ...
    spec = ConstructionSpec(
        ...
        robot=robot,
        is_urdf=am.is_urdf,
        package_roots={key: str(value) for key, value in am.package_roots.items()},
...
class _ArmSampler:
    ...
    def __init__(self, spec: ConstructionSpec) -> None:
        ...
        self.model = compile_model(
            ...
            package_roots=dict(spec.package_roots),
        )
...
def cli_main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="default: <state dir>/reachability/<robot>_capability.npz",
    )
    ...
    # data/ is read-only LFS territory; built maps are generated state.
    out = args.out or STATE_DIR / "reachability" / f"{args.robot}_capability.npz"
    cap.save(out)
    print(out)
