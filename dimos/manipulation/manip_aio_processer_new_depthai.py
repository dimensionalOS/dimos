...
class ManipulationProcessor:
    ...
    def process_frame(
        ...
        try:
                ...
                per_camera_data.append(
                    {
                        ...
                        "segmentation_viz": per_camera_segmentation_results[cam_idx].get(
                            "viz_frame"
                        ),
                    }
                )
            ...
            print(
                f"Depth range: {depth_images[viz_camera_idx].min():.2f}m to {depth_images[viz_camera_idx].max():.2f}m"
            )
            print(
                f"Base image shape: {base_image.shape}, range: [{base_image.min()}, {base_image.max()}]"
            )
            ...
            camera_objects_for_viz = self.transform_objects_to_camera(
                all_objects, self.camera_configs[viz_camera_idx]["extrinsics"]
            )

            camera_detected_for_viz = self.transform_objects_to_camera(
                detected_objects, self.camera_configs[viz_camera_idx]["extrinsics"]
            )
    ...
    def run_object_detection(self, rgb_image: np.ndarray) -> dict[str, Any]:
        ...
        try:
            ...
            # Create dimos Image
            dimos_bgr = Image.from_numpy(bgr_image, format=ImageFormat.BGR, timestamp=time.time())
            ...
            for detection in detection_results.detections:
                ...
                confidences.append(
                    detection.confidence if hasattr(detection, "confidence") else 1.0
                )
    ...
    def create_detection_visualization(
        ...
        for i in range(len(bboxes)):
            if len(bboxes[i]) >= 4:
                ...
                cv2.rectangle(
                    viz_image, (x1, y1 - label_size[1] - 4), (x1 + label_size[0], y1), color, -1
                )
    ...
    def run_grasp_generation(self, filtered_objects: list[dict], full_pcd) -> list[dict]:
        ...
        try:
            ...
            all_grasps = self.grasp_generator.generate_grasps_from_objects(valid_objects, full_pcd)
                    ...
                    if i in valid_indices:
                        ...
                        if obj_pcd and len(obj_pcd.points) > 0:
                            ...
                            if i == 0:  # Debug first object only
                                ...
                                print("[DEBUG] First 5 grasp positions:")
                            ...
                            if i == 0:
                                print(
                                    f"[DEBUG] Minimum grasp distance for object 0: {min_distance:.3f}m"
                                )

                            obj_with_grasps["grasps"] = object_grasps
                            if object_grasps:
                                logger.info(
                                    f"Object {i} ({obj.get('class_name', 'unknown')}): {len(object_grasps)} grasps assigned"
                                )
                        else:
                ...
                if total_assigned == 0:
                    ...
                    if len(valid_objects) > 0 and len(all_grasps) > 0:
                        ...
                        if obj_pcd and len(obj_pcd.points) > 0:
                            ...
                            print(
                                f"  Min distance: {min(distances):.3f}m, Max distance: {max(distances):.3f}m"
                            )
    ...
    def transform_pointcloud_to_world(
        self, pointcloud: o3d.geometry.PointCloud, extrinsics: np.ndarray
    ...
    def transform_objects_to_camera(
        self, world_objects: list[dict], extrinsics: np.ndarray
    ) -> list[dict]:
        """
    ...
    def merge_multi_view_detections(self, all_objects: list[dict]) -> list[dict]:
        """
    ...
    def objects_are_duplicate_3d(self, obj1: dict, obj2: dict, threshold: float = 0.05) -> bool:
