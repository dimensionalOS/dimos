...
def test_real_roboplan_plans_fixed_orientation_cartesian_path(
    ...
    if not Path(config.urdf_path).exists():
        pytest.skip(f"xArm model is unavailable: {config.urdf_path}")
...
def test_real_roboplan_synchronizes_different_length_dual_arm_targets(
    ...
    if not Path(left_config.urdf_path).exists():
        pytest.skip(f"xArm model is unavailable: {left_config.urdf_path}")
