...
class ViserVisualizationConfig(BaseModel):
    ...
    # Tip links to expose as Cartesian gizmos. Empty -> single gizmo on the robot's
    # end_effector_link (default). Two+ links -> one gizmo per tip, all solved together
    # via multi-target IK (bimanual). e.g. ["left_arm_link7", "right_arm_link7"].
    pose_target_links: list[str] = Field(
        default_factory=list,
        validation_alias=AliasChoices("pose_target_links", "viser_pose_target_links"),
    )
