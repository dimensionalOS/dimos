...
class VLMTaskPlanner:
    ...
    def _query_vlm_for_object(
        self, rgb_image: np.ndarray, instruction: str, available_objects: list[dict]
    ) -> str:
        """Query VLM to identify target object."""
...
Now identify and click on: {instruction}"""
    ...
    def _parse_pixel_from_vlm(self, vlm_response: str) -> tuple[int, int] | None:
    ...
    def plan_place_task(
        self, instruction: str, rgb_image: np.ndarray, depth_image: np.ndarray
    ) -> dict | None:
        """
    ...
    def plan_complex_task(self, instruction: str, rgb_image: np.ndarray) -> list[TaskPlan]:
        """
    ...
    def validate_grasp_reachability(self, grasp: dict) -> bool:
        ...
        workspace_limits = {"x": (-0.5, 0.5), "y": (-0.5, 0.5), "z": (0.1, 0.8)}

        return (
            workspace_limits["x"][0] <= translation[0] <= workspace_limits["x"][1]
            and workspace_limits["y"][0] <= translation[1] <= workspace_limits["y"][1]
            and workspace_limits["z"][0] <= translation[2] <= workspace_limits["z"][1]
        )
