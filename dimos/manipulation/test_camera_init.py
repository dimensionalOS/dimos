    ...
    while True:
        ...
        if frames_ready and len(color_frames) == 2:
            ...
            depth1_vis = cv2.applyColorMap(
                cv2.convertScaleAbs(depth_frames[0], alpha=0.03),
                cv2.COLORMAP_JET
            )
            depth2_vis = cv2.applyColorMap(
                cv2.convertScaleAbs(depth_frames[1], alpha=0.03),
                cv2.COLORMAP_JET
            )
            ...
            # Add labels
            cv2.putText(color1_display, f"Cam1 Color [{device_infos[0].getMxId()[:8]}]", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.putText(depth1_display, "Cam1 Depth", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            cv2.putText(color2_display, f"Cam2 Color [{device_infos[1].getMxId()[:8]}]", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.putText(depth2_display, "Cam2 Depth", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
