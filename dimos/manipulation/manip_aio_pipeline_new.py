...
class ManipulationPipeline:
    ...
    def __init__(
        ...
        self.latest_frames = {
            cam_id: {"rgb": None, "depth": None} for cam_id in range(self.num_cameras)
        }
        self.frame_locks = {cam_id: threading.Lock() for cam_id in range(self.num_cameras)}
    ...
    def create_streams(self, camera_streams: list[rx.Observable]) -> dict[str, rx.Observable]:
        ...
        if len(camera_streams) != self.num_cameras:
            raise ValueError(
                f"Expected {self.num_cameras} camera streams, got {len(camera_streams)}"
            )
        ...
        def process_synchronized_frames(trigger_frame) -> dict | None:
            ...
            try:
                ...
                processed = self.processor.process_frame(
                    rgb_images=rgb_images, depth_images=depth_images, generate_grasps=False
                )
        ...
        processing_stream = trigger_stream.pipe(
            ops.map(process_synchronized_frames), ops.filter(lambda x: x is not None), ops.share()
        )

        # Visualization streams
        detection_viz_stream = processing_stream.pipe(
            ops.map(
                lambda x: x.get("per_camera_data", [{}])[0].get("detection_viz")
                if x.get("per_camera_data")
                else None
            ),
        ...
        pointcloud_viz_stream = processing_stream.pipe(
            ops.map(lambda x: x.get("pointcloud_viz")), ops.filter(lambda x: x is not None)
        )

        detected_viz_stream = processing_stream.pipe(
            ops.map(lambda x: x.get("detected_pointcloud_viz")), ops.filter(lambda x: x is not None)
        )

        misc_viz_stream = processing_stream.pipe(
            ops.map(lambda x: x.get("misc_pointcloud_viz")), ops.filter(lambda x: x is not None)
        )
        ...
        objects_stream = processing_stream.pipe(
            ops.map(
                lambda x: [
                    obj
                    for cam_data in x.get("per_camera_data", [])
        ...
        return {
            ...
            "full_pointcloud": processing_stream.pipe(
                ops.map(lambda x: x.get("full_pointcloud")), ops.filter(lambda x: x is not None)
            ),
        }
    ...
    def _create_object_grasp_task(
        ...
        try:
            task = asyncio.run_coroutine_threadsafe(
                self._send_object_grasp_request(obj_id, points, colors), self.grasp_loop
            )
            return task
        except Exception as e:
    ...
    def _draw_object_grasps(
        ...
        for grasp in grasps:
            ...
            if translation[2] > 0:  # Valid depth
                ...
                cv2.putText(image, score_text, (x + 10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
