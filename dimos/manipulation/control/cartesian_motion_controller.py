...
class CartesianMotionControllerConfig(ModuleConfig):
    ...
    orientation_kp: float = 0.5  # Proportional gain
    orientation_ki: float = 0.0  # Integral gain
    orientation_kd: float = 0.3  # Derivative gain
    ...
    max_position_error: float = 1.0  # m - max allowed position error before emergency stop
    max_orientation_error: float = 3.15 #1.57  # rad (~90°) - max allowed orientation error
...
class CartesianMotionController(Module):
    ...
    def set_gripper_position(self, position: float = 10) -> int:
        """
        Set gripper position through the arm driver.
        
        Args:
            position: Gripper position (0-850 where 0=closed, 850=open)
        """
        try:
            # arm_driver.set_gripper_position returns (code, message) tuple!
            result_tuple = self.arm_driver.set_gripper_position(
                position=position, 
                wait=True,  # Wait for gripper to reach position
                speed=5000  # Optional speed
            )
            
            code = result_tuple[0] if isinstance(result_tuple, tuple) else result_tuple
            message = result_tuple[1] if isinstance(result_tuple, tuple) and len(result_tuple) > 1 else ""
            
            logger.info(f"Gripper command: position={position}, code={code}, msg={message}")
            return code
        except Exception as e:
    ...
    def open_gripper(self) -> int:
        ...
        return self.set_gripper_position(850)  # 850 = fully open

    @rpc
    def close_gripper(self) -> int:
        """Fully close the gripper."""
        return self.set_gripper_position(0)  # 0 = fully closed (not 10!)
