    ...
    for i, device_info in enumerate(device_infos[:2]):
        try:
            ...
            camera_configs.append(
                {"camera_id": i, "intrinsics": [fx, fy, cx, cy], "extrinsics": extrinsics}
            )
...
def test_pipeline_single_frame():
    ...
    with contextlib.ExitStack() as stack:
        ...
        for i in range(len(rgb_images)):
            print(
                f"Camera {i}: RGB shape={rgb_images[i].shape}, Depth shape={depth_images[i].shape}"
            )
            if rgb_images[i].shape[:2] != depth_images[i].shape[:2]:
                print("  ⚠️ Size mismatch! Resizing depth to match RGB...")
