...
class ManipulationModule(Module):
    ...
    def __init__(
        ...
        vlm_model: QwenVlModel | None = None,
        **kwargs,
    ):
        """
    ...
    def start_camera_streams(
        self, camera_stream_0: rx.Observable, camera_stream_1: rx.Observable | None = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            self.pipeline_streams["objects_with_grasps"].subscribe(
                on_next=lambda objs: logger.debug(
                    f"Pipeline detected {len(objs)} graspable objects"
                )
            )
    ...
    def pick_task(self, instruction: str) -> dict[str, Any]:
        ...
        try:
            ...
            task_plan = self.planner.plan_pick_task(
                instruction=instruction, rgb_image=rgb_image, retry_on_failure=True
            )
            ...
            logger.info(
                f"VLM identified: {task_plan.target_object.get('class_name', 'unknown')} "
            ...
            gripper_width = min(object_width + self.grasp_width_offset, self.gripper_max_opening)

            # Store current object info
            self.current_object = task_plan.target_object

            logger.info("Pick task planned successfully:")
    ...
    def place_task(self, instruction: str) -> dict[str, Any]:
        ...
        try:
            ...
            place_plan = self.planner.plan_place_task(
                instruction=instruction, rgb_image=rgb_image, depth_image=depth_image
            )
            ...
            place_orientation = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]

            logger.info("Place task planned successfully:")
    ...
    def pick_and_place_sequence(
        self, pick_instruction: str, place_instruction: str
    ) -> dict[str, Any]:
        """
        ...
        """
        logger.info("Planning pick and place sequence:")
        ...
        return {
            ...
            "execution_sequence": [
                {
                    "action": "move",
                    "position": pick_result["approach_position"],
                    "description": "Approach object",
                },
                {
                    "action": "open_gripper",
                    "width": pick_result["gripper_width"],
                    "description": "Open gripper",
                },
                {
                    "action": "move",
                    "position": pick_result["position"],
                    "description": "Move to grasp",
                },
                {"action": "close_gripper", "description": "Grasp object"},
                {
                    "action": "move",
                    "position": pick_result["retract_position"],
                    "description": "Lift object",
                },
                {
                    "action": "move",
                    "position": place_result["approach_position"],
                    "description": "Move to place area",
                },
                {
                    "action": "move",
                    "position": place_result["position"],
                ...
                {
                    "action": "move",
                    "position": place_result["retract_position"],
                    "description": "Retract from place",
                },
            ],
        }
    ...
    def _validate_position(self, position: list) -> bool:
        ...
        if not (self.workspace_min_z <= z <= self.workspace_max_z):
            logger.error(
                f"Position z={z:.3f}m outside limits "
        ...
        if not (self.workspace_min_radius <= distance <= self.workspace_max_radius):
            logger.error(
                f"Position distance={distance:.3f}m outside limits "
    ...
    def set_object_held(self, held: bool) -> dict[str, Any]:
        ...
        return {
            ...
            "current_object": self.current_object.get("class_name")
    ...
    def get_detected_objects(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "success", "num_objects": len(object_list), "objects": object_list}
