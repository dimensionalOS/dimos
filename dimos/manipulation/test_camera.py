...
class DualOAKSystem:
    def __init__(self):
        self.frames = {
            'cam1_rgb': None, 'cam1_depth': None,
            'cam2_rgb': None, 'cam2_depth': None
        }
    ...
    def test_single_camera(self, device_info, cam_id):
        ...
        class TestDisplay(dai.node.HostNode):
            ...
            def process(self, rgb: dai.ImgFrame, depth: dai.ImgFrame):
                ...
                # Add camera ID
                cv2.putText(combined, f"Camera {cam_id}", (10, 30),
        ...
        with p:
            display = p.create(TestDisplay).build(rgb_out, stereo.depth)
    ...
    def run_dual_cameras(self, device_info1, device_info2):
        ...
        def run_camera(device_info, cam_id):
            ...
            with p:
                display = p.create(lambda: DualDisplay(self, cam_id)).build(
                    rgb_out, stereo.depth
                )
                device = dai.Device(p, device_info)
        ...
        while not self.stop_flag:
            if all(self.frames[k] is not None for k in self.frames):
                ...
                # Add labels
                cv2.putText(combined, "Cam1 RGB", (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(combined, "Cam1 Depth", (650, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(combined, "Cam2 RGB", (10, 510), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(combined, "Cam2 Depth", (650, 510), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
