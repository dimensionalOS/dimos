...
class PinkIK:
    ...
    def solve_pose_targets(
        ...
        world: WorldSpec,
        robot_id: WorldRobotID,
        pose_targets: Mapping[str, PoseStamped],
        *,
        seed: JointState | None = None,
        ...
        """Solve IK for multiple end-effector tips at once (e.g. bimanual).

        ``pose_targets`` maps tip link names to world-frame poses. Every tip is
        constrained simultaneously over the robot's controlled joints, yielding one
        combined ``IKResult`` whose ``joint_state`` spans ``config.joint_names``.
        """
        if not world.is_finalized:
            return _failure(IKStatus.NO_SOLUTION, "World must be finalized before IK")
        if not pose_targets:
            return _failure(IKStatus.NO_SOLUTION, "No pose targets supplied")
        try:
            robot_context = self._get_robot_context(world, robot_id)
        except (FileNotFoundError, ImportError, ValueError) as exc:
            return _failure(IKStatus.NO_SOLUTION, f"Pink IK model setup failed: {exc}")
        ...
        try:
            targets_model = {
                tip: self._target_in_model_frame(config, pose) for tip, pose in pose_targets.items()
            }
            frame_ids = {tip: _get_frame_id(robot_context.model, tip) for tip in pose_targets}
        except ValueError as exc:
            return _failure(IKStatus.NO_SOLUTION, f"Pink IK target setup failed: {exc}")
    ...
    def _solve_multi(
        self,
        robot_context: _PinkRobotContext,
        targets_model: Mapping[str, NDArray[np.float64]],
        ...
        tasks: list[Any] = []
        for tip, target_model in targets_model.items():
            frame_task = pink.tasks.FrameTask(
                tip,
                position_cost=self.config.position_cost,
                orientation_cost=self.config.orientation_cost,
            ...
            frame_task.set_target(_matrix_to_se3(self._modules.pinocchio, target_model))
        ...
        worst_position_error = float("inf")
        worst_orientation_error = float("inf")
        for iteration in range(self.config.max_iterations):
            worst_position_error = 0.0
            worst_orientation_error = 0.0
            for tip, target_model in targets_model.items():
                current_pose = self._frame_matrix(robot_context, configuration.q, frame_ids[tip])
                ...
                worst_orientation_error = max(worst_orientation_error, orientation_error)
            if (
            ...
            if not _within_limits(joint_positions, lower_limits, upper_limits):
                return IKResult(
                    ...
                    orientation_error=worst_orientation_error,
                    iterations=iteration + 1,
                    message="Pink IK candidate violates DimOS joint limits",
                )
        return IKResult(
