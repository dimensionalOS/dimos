...
def test_permuted_multi_group_path_is_reordered_to_selection() -> None:
    output = JointTrajectory(
        joint_names=["arm/a", "arm/b"],
        points=[
            TrajectoryPoint(time_from_start=0.0, positions=[0.0, 1.0], velocities=[0.0, 0.0]),
            TrajectoryPoint(time_from_start=1.0, positions=[0.4, 3.0], velocities=[0.0, 0.0]),
        ],
    )
    parametrizer = _FixedParametrizer(output)
    permuted = [
        JointState(name=["arm/b", "arm/a"], position=[1.0, 0.0], velocity=[0.1, 0.2]),
        JointState(name=["arm/b", "arm/a"], position=[3.0, 0.4], velocity=[0.3, 0.4]),
    ]

    plan = parametrizer.materialize_plan(
        MagicMock(spec=WorldSpec),
        _selection(),
        PlanningResult(status=PlanningStatus.SUCCESS, path=permuted),
    )

    assert [list(state.name) for state in plan.path] == [["arm/a", "arm/b"]] * 2
    assert plan.path[0].position == [0.0, 1.0]
    assert plan.path[1].position == [0.4, 3.0]
    assert plan.path[0].velocity == [0.2, 0.1]
