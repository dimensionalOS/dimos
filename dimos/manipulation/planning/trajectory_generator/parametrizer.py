...
class BaseTrajectoryParametrizer(ABC):
    ...
    def materialize_plan(
        ...
        path = self._path_in_selection_order(
            [JointState(state) for state in result.path], selection.joint_names
        )
    ...
    @classmethod
    def _path_in_selection_order(
        cls,
        path: list[JointState],
        expected_names: Sequence[str],
    ) -> list[JointState]:
        """Reorder waypoints that arrive in a generated group's canonical order.

        Multi-group plans come back in the composite group's joint order,
        which may be a permutation of the requested selection.
        """
        expected = list(expected_names)
        reordered: list[JointState] = []
        for state in path:
            names = list(state.name)
            if names == expected or sorted(names) != sorted(expected):
                reordered.append(state)
                continue
            order = [names.index(name) for name in expected]
            fields = {
                "name": list(expected),
                "position": [state.position[index] for index in order],
            }
            if len(state.velocity) == len(names):
                fields["velocity"] = [state.velocity[index] for index in order]
            reordered.append(JointState(fields))
        return reordered
