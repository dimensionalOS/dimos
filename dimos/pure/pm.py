...
House rules: re-export *names* only — never submodules, no star imports. The
``dimos.pure.typing`` submodule stays package-qualified and must never shadow
stdlib ``typing``. Everything here is the zero-engine data layer; drivers,
alignment, and the rim import *us*, never the reverse.
"""
