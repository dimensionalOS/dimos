...
def test_mealy_state_threads_through_over():
    samples = [Sample(ts=1.0, v=1), Sample(ts=2.0, v=2), Sample(ts=3.0, v=3)]
    rows = list(Accum().over(sample=samples))
    assert [r.total for r in rows] == [1, 3, 6]  # replace() accumulated across ticks
    assert [r.ts for r in rows] == [1.0, 2.0, 3.0]  # engine stamps from the tick row
