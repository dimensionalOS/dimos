...
"""Static typing surface for pure modules.
...
# ``tf`` / ``debug`` are typed ``Any`` at this layer: their concrete types
# (``TfSource``, ``Debug``) live in the tf buffer and the debug recorder, which
# import this module — the annotations tighten when those land.
TfSource: TypeAlias = Any
Debug: TypeAlias = Any
...
class _InAccessor:
    ...
    def __get__(self, obj: Any, owner: type | None = None) -> Any:
        ...
        raise NotImplementedError(  # RIM SEAM — body lands with dimos.pure.rim
            "m.i needs live ports; the rim is not part of this layer yet"
        )
...
class _OutAccessor:
    ...
    def __get__(self, obj: Any, owner: type | None = None) -> Any:
        if obj is None:
            return self  # class access: the accessor itself (first overload)
        raise NotImplementedError(  # RIM SEAM — body lands with dimos.pure.rim
            "m.o needs live ports; the rim is not part of this layer yet"
        )
...
class EngineSurface:
    ...
    def over(
        ...
        """Align streams, drive the step, yield typed Out rows."""
        from dimos.pure.stepspec import step_spec  # lazy: sanctioned edge #1

        step_spec(type(self))  # classification gate — raises on an unclassified class
        raise NotImplementedError(  # ENGINE SEAM — body lands with align + drivers
            "over() needs the aligner and the driver; they are not part of this layer yet"
        )
