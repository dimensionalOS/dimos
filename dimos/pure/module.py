...
Deliberately absent here: ``step``, ``fold``, ``In``, ``Out``, ``State``
(classification discovers them on subclasses); the runtime face
(``over``/``i``/``o``, ``warmup``/``start``/``stop``, ``__call__``,
``blueprint``) lands with the engine and rim.
"""
...
def _resolve_tf_frames(
    ...
    """Resolve declared frame templates against instance config."""
    cls = type(module)
