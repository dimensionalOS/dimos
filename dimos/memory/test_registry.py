...
from dimos.memory.blobstore.sqlite import SqliteBlobStore, SqliteBlobStoreConfig
from dimos.memory.notifier.subject import SubjectNotifier
from dimos.memory.observationstore.sqlite import SqliteObservationStoreConfig
from dimos.memory.registry import RegistryStore, deserialize_component, qual
from dimos.memory.store.sqlite import SqliteStore
from dimos.memory.vectorstore.sqlite import SqliteVectorStore, SqliteVectorStoreConfig


class TestQual:
    def test_qual_blob_store(self) -> None:
        assert qual(SqliteBlobStore) == "dimos.memory.blobstore.sqlite.SqliteBlobStore"

    def test_qual_file_blob_store(self) -> None:
        assert qual(FileBlobStore) == "dimos.memory.blobstore.file.FileBlobStore"

    def test_qual_vector_store(self) -> None:
        assert qual(SqliteVectorStore) == "dimos.memory.vectorstore.sqlite.SqliteVectorStore"
...
class TestRegistryStore:
    def test_put_get_round_trip(self, tmp_path) -> None:
        from dimos.memory.utils.sqlite import open_sqlite_connection
