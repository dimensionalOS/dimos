...
def load_robot_meshes(
    ...
    *,
    ...
    ``meshdir`` points MuJoCo at a local mesh directory when an MJCF
    uses bare mesh filenames.

    ``assets`` is an optional ``{filename: bytes}`` map for embedded assets.
    """
    path = Path(mjcf_path)
    xml = path.read_text()
    if meshdir is not None:
        ...
        for include in root.iter("include"):
            ...
            if include_file and not Path(include_file).is_absolute():
                include.set("file", str((path.parent / include_file).resolve()))
