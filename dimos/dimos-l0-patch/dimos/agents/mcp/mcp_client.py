...
from dimos.agents.mcp.mcp_server import is_loopback_host
...
class McpClient(Module):
    ...
    def start(self) -> None:
        ...
        # A non-loopback MCP server requires bearer auth; send the token from
        # the same GlobalConfig field the server checks, so client and server
        # share one configuration. Loopback stays tokenless.
        mcp_host = urlparse(self.config.mcp_server_url).hostname
        if global_config.mcp_auth_token and mcp_host and not is_loopback_host(mcp_host):
            self._http_client.headers["authorization"] = (
                f"Bearer {global_config.mcp_auth_token}"
            )
