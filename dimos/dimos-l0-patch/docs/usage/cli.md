...
| `--mcp-auth-token` | TEXT | `None` | Bearer token required from non-loopback MCP clients. Mandatory when `--mcp-host` is not loopback, otherwise the server refuses to start |
| `--mcp-allowed-origins` | TEXT | `[]` | Comma-separated CORS allowlist for the MCP endpoint (empty = deny all cross-origin browser requests) |
...
By default the MCP server binds to `127.0.0.1`, requires no token, and denies all
cross-origin browser requests (empty CORS allowlist). Loopback clients — `dimos mcp
call`, Claude Code on the same machine — work unchanged.

To expose the MCP endpoint to other machines:

1. Set a bearer token: `DIMOS_MCP_AUTH_TOKEN=<secret>` (env var or `.env`).
2. Bind a non-loopback address: `dimos --mcp-host 0.0.0.0 run ...`.

Binding a non-loopback address **without** a token refuses to start (fail-closed).
Remote clients must send `Authorization: Bearer <secret>` on every request
(`POST /mcp` and the `GET /mcp` SSE channel alike); `McpClient` adds the header
automatically when its `mcp_server_url` is not loopback, reading the same
`DIMOS_MCP_AUTH_TOKEN` configuration as the server. The `agent_send` tool is
restricted to loopback callers regardless of token.

If a browser-based UI needs access, whitelist its origin explicitly:
`DIMOS_MCP_ALLOWED_ORIGINS=http://localhost:3000,https://ui.example.com`. Request
bodies larger than 1 MB are rejected with `413`.
