...
class GStreamerVideoSenderWithMetadata:
    ...
    def create_pipeline(self):
        ...
        # Optional: Add visible timestamp overlay for debugging
        timeoverlay = Gst.ElementFactory.make("timeoverlay", "overlay")
        timeoverlay.set_property("halignment", "left")
        timeoverlay.set_property("valignment", "top")
        ...
        if not videoconvert.link(timeoverlay):
            raise RuntimeError("Failed to link videoconvert to timeoverlay")
        if not timeoverlay.link(self.encoder):
    ...
    def _inject_timestamp_metadata(self, pad, info, user_data):
        """Inject timestamp metadata into video frames."""
        ...
        if buffer:
            ...
            metadata = {
                ...
                "capture_time_ns": int(current_time * 1e9),
            }

            # Add metadata to buffer
            # Note: In a production system, you'd use proper SEI NAL units
            # For now, we'll set the buffer timestamp
            ...
            # Log periodically
            if self.frame_count % (self.framerate * 2) == 0:
                logger.info(
                    f"Injected metadata for frame {self.frame_count}, time: {current_time:.3f}"
                )
