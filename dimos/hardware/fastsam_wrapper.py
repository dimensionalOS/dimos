...
class FastSAMWrapper:
    ...
    def segment_with_points(self, image, points, point_labels, conf=0.25, iou=0.7):
        ...
        everything_results = self.fastsam_model(
            temp_img_path, device=self.device, retina_masks=True, imgsz=1024, conf=conf, iou=iou
        )
        ...
        print(
            f"Using {len(points)} points: {len([l for l in point_labels if l == 1])} foreground, {len([l for l in point_labels if l == 0])} background"
        )
        ...
        if ann is not None:
            if isinstance(ann, np.ndarray):
                print(
                    f"Direct array shape: {ann.shape}, dtype: {ann.dtype}, unique values: {np.unique(ann)[:10]}"
                )
