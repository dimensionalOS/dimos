...
class ForceTorqueCalibrator:
    ...
    def load_calibration_data(self, csv_path: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        print(
            f"  Force magnitude mean:  {np.linalg.norm(force_torque_data[:, :3], axis=1).mean():.2f} N"
        )
        print(
            f"  Torque magnitude mean: {np.linalg.norm(force_torque_data[:, 3:], axis=1).mean():.4f} N⋅m"
        )

        return sensor_data, force_torque_data

    def calculate_calibration_matrix(
        self, sensor_data: np.ndarray, force_torque_data: np.ndarray, use_bias: bool = True
        ...
        if use_bias:
            ...
            X, residuals, rank, singular_values = np.linalg.lstsq(
                S_augmented, force_torque_data, rcond=None
            )
            ...
            # C^T = pinv(S) @ F
            X, residuals, rank, singular_values = np.linalg.lstsq(
                sensor_data, force_torque_data, rcond=None
            )
    ...
    def apply_calibration(self, sensor_data: np.ndarray) -> np.ndarray:
        ...
        if sensor_data.shape[-1] != self.sensor_channels:
            raise ValueError(
                f"Expected {self.sensor_channels} sensor channels, got {sensor_data.shape[-1]}"
            )
    ...
    def run_live_mode(
        self, calibration_file: str, zmq_in_port: int = 5555, zmq_out_port: int = 5556
    ):
        """
        ...
        print("-" * 80)
        print(
            "Time         Fx(N)    Fy(N)    Fz(N)    Mx(N⋅m)  My(N⋅m)  Mz(N⋅m)  |F|(N)   |M|(N⋅m)"
        )
        print("-" * 80)
                    ...
                    if sub_socket.poll(0):  # 100ms timeout
                        ...
                        if "sensor_moving_averages" in data:
                            ...
                            if len(sensor_values) == self.sensor_channels:
                                ...
                                print(
                                    f"{timestamp}  {force_torque[0]:8.3f} {force_torque[1]:8.3f} "
                                ...
                                print(
                                    f"Warning: Expected {self.sensor_channels} sensors, got {len(sensor_values)}"
                                )
...
Examples:
  ...
  python calc_calibration_matrix.py --csv data.csv --output cal.json --no-bias
    ...
    parser.add_argument(
        "--output", type=str, help="Output file for calibration matrix (.json or .npz)"
    )
    ...
    parser.add_argument(
        "--in-port", type=int, default=5555, help="ZMQ port for input sensor data (default: 5555)"
    )
    parser.add_argument(
        "--out-port",
        type=int,
        default=5556,
        help="ZMQ port for output calibrated data (default: 5556)",
    )
    parser.add_argument(
        "--no-bias", action="store_true", help="Calculate calibration without bias term"
    )
    ...
    if args.live:
        ...
        for i, weight in enumerate(sensor_weights):
            ...
            print(
                f"  Sensor {i + 1:2d} (Magnet {magnet_num}, Sensor {sensor_in_magnet}): {weight:.4f}"
            )
