...
class OpenArmBus:
    ...
    def _send_raw(self, arbitration_id: int, data: bytes) -> None:
        ...
        # Retry on TX buffer full (ENOBUFS / errno 105) — the gs_usb
        # adapter has a tiny kernel-side TX queue. A short backoff lets
        # the kernel drain one frame before we try again.
        for attempt in range(4):
            try:
                self._bus.send(msg)
                return
            except can.CanOperationError as e:
                if "105" in str(e) and attempt < 3:
