...
"""SHM-based MuJoCo ``WholeBodyAdapter`` for the Unitree G1.
...
Replaces the older ``SimMujocoG1WholeBodyAdapter`` which spawned its own
``MujocoConnection`` subprocess — the in-process engine path lets the
sim share state with everything else in the blueprint (lidar publisher,
splat camera, manipulation, ...) without two competing physics worlds.
"""
...
class ShmMujocoG1WholeBodyAdapter:
    ...
    def __init__(
        ...
        **_: Any,
        ...
        if address is None:
            raise ValueError(
                "ShmMujocoG1WholeBodyAdapter: address (MJCF XML path) is required — "
    ...
    def connect(self) -> bool:
        ...
        while True:
            try:
                ...
                if time.monotonic() > deadline:
                    logger.error(
                        "ShmMujocoG1WholeBodyAdapter: SHM buffers not found",
        ...
        while not self._shm.is_ready():
            if time.monotonic() > deadline:
                logger.error(
                    "ShmMujocoG1WholeBodyAdapter: sim module not ready",
        ...
        logger.info(
            "ShmMujocoG1WholeBodyAdapter connected",
    ...
    def write_motor_commands(self, commands: list[MotorCommand]) -> bool:
        ...
        if len(commands) != _NUM_MOTORS:
            logger.error(
                f"ShmMujocoG1WholeBodyAdapter: expected {_NUM_MOTORS} commands, got {len(commands)}"
            )
            return False
        # Flatten the per-motor command into per-joint arrays.  POS_STOP
        # ("no command") is replaced with 0.0 — mirrors the real-hw G1
        # adapter's behavior; the engine's PD only acts when kp > 0 anyway.
...
def register(registry: WholeBodyAdapterRegistry) -> None:
    ...
    registry.register("sim_mujoco_engine_g1", ShmMujocoG1WholeBodyAdapter)


__all__ = ["ShmMujocoG1WholeBodyAdapter"]
