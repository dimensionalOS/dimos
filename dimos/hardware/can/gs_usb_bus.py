...
SocketCAN is Linux-only; this bus drives gs_usb-protocol adapters
(candlelight class: CANable 2.0, the HHS "CANFD Analyser" Galaxea ships,
etc.) entirely from userspace over libusb, so CAN arms run natively on
macOS. Validated on an M4 Pro at a sustained 250 Hz control loop with the
Galaxea A1Z (p99 cycle 5.2 ms, 0/7500 cycles over the SDK's 12.5 ms limit,
~100% feedback from all 7 motors).

Device quirks handled here:
- TX endpoint is discovered from the descriptors (some adapters use 0x01;
...
- RX runs on a dedicated reader thread (see _rx_loop): these adapters are
  Full-Speed USB devices read one frame per libusb round-trip, and a
  high-rate control loop budgets only ~1 ms/cycle for draining.
  Synchronous reads from that budget cannot keep up with the thousands of
  frames/s the bus carries (motor feedback plus TX echoes), so the device
  FIFO overflows and feedback freezes for hundreds of ms (observed on A1Z
  hardware: frozen joints in teach recordings, "CAN feedback stale" during
  replay). The reader thread drains USB continuously - libusb releases the
  GIL while blocked - and recv() becomes a queue pop that always meets the
  control loop's budget.

Requires ``gs-usb``, ``pyusb``, and system libusb (macOS:
``brew install libusb``). Callers supply their adapter's USB IDs — e.g.
Galaxea's HHS analyser (0xA8FA:0x8598) or a candlelight CANable 2.0
(0x1D50:0x606F).

Promoted from ``dimos/hardware/manipulators/galaxea_a1z/`` when OpenYAM
became its second user; the A1Z module now wraps this one with its
vendor defaults.
"""
...
# Reader-thread queue depth. At a 7-motor arm's ~1750 feedback frames/s this
# holds multiple seconds of backlog; the consumer normally keeps the queue
# near-empty, and on overflow the oldest frames are dropped so recv() keeps
# returning fresh state instead of replaying stale history.
...
def _initialize_ready_usb_device(
    ...
    After ``GsUsb.stop()``, these adapters briefly re-enumerate on macOS.
    During that window PyUSB can find the descriptor while configuration or
    control transfers still raise ``USBError(ENOENT, "Entity not found")``.
    Retry the complete initialization so immediate DimOS restarts are
    reliable.
    """
...
class GsUsbMacBus(can.BusABC):
    ...
    def _rx_loop(self) -> None:
        ...
        the control thread and the rest of the process compete for the
    ...
    def _recv_internal(self, timeout: float | None) -> tuple[can.Message | None, bool]:
        # High-rate feedback drains call recv(timeout=0.0) in a tight loop
