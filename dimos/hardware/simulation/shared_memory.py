...
def shm_key_from_path(config_path: Path | str) -> str:
    """Derive a deterministic short key from a simulation model path.

    Both simulation provider and adapter compute the same key from the same path,
...
class ManipShmWriter:
    ...
    Owned by the active simulation provider. Creates the SHM buffers on init and
