...
def _record_session(db_path: Path) -> None:
    ...
    # Every declared In port needs a transport on this base; only three
    # carry test traffic.
    transports = {
        name: _DirectTransport()
        for name in (
            "color_image",
            "chest_image",
            "left_hand_image",
    ...
    counts = {name: 0 for name in ("color_image", "coordinator_joint_state", "status")}
