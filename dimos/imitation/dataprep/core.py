...
def iter_episode_samples(
    ...
    # Streams whose messages carry joint names can vary in width within one
    # episode (e.g. commanded targets cover only the joints commanded that
    # tick). Align those onto the union of names, holding missing joints at
    # their previous value, so frames stack into one fixed-width array.
