...
from dimos.mapping.benchmark.tool_benchmark import (
    classify_bounded_vs_unbounded,
    compute_run_metrics,
    load_jsonl,
)
...
def _row_to_metrics(row: Mapping[str, str]) -> RunMetrics:
    ...
    return RunMetrics(
        ...
        self_consistency_rmse_m=_opt_float(row, "self_consistency_rmse_m"),
        self_consistency_n_matched=_int(row, "self_consistency_n_matched"),
        ...
        max_checkpoint_error_m=_opt_float(row, "max_checkpoint_error_m"),
        checkpoint_error_rms_m=_opt_float(row, "checkpoint_error_rms_m"),
        checkpoint_count=_int(row, "checkpoint_count"),
        recovery_time_s=_opt_float(row, "recovery_time_s"),
    )
...
def render_results_md(entries: list[tuple[str, RunMetrics]], csv_path: Path) -> str:
    lines = [
        ...
        "",
        "PRIMARY metric is **Return error (vs fiducial GT)** -- position + heading error "
        "against a marker tag withheld from every localization map under test, the only "
        "ground-truth-referenced number this benchmark can honestly claim (see "
        "benchmark-spec.md #4). Self-consistency is odom-vs-corrected agreement only, no "
        'external reference -- read it as "how much did correction move the estimate," '
        "never as ATE.",
        "",
        "## All runs",
        "",
        "| Route | Mode | Loop-closure error | Return error (vs fiducial GT) | Max checkpoint "
        "err | Checkpoint RMS | Recovery (kidnap) | Self-consistency | Corrections | Notes | "
        "Timestamp |",
        "|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    for ts, m in sorted(entries, key=lambda e: e[0], reverse=True):
        ...
        lines.append(
            ...
            f"{_fmt_max_checkpoint(m)} | {_fmt_checkpoint_rms(m)} | {_fmt_recovery_time(m)} | "
            f"{_fmt_self_consistency(m)} | {corr} | {notes} | {ts} |"
        )
    lines.append("")
    ...
    if h2h:
        ...
        for route in sorted(h2h):
            ...
            lines.append("")
            lines.append(
                "| Mode | Loop-closure error | Return error (vs fiducial GT) | Self-consistency "
                "| Corrections |"
            )
            lines.append("|---|---|---|---|---|")
            for mode in sorted(latest, key=lambda mo: (_MODE_ORDER.get(mo, len(_MODE_ORDER)), mo)):
                ...
                lines.append(
                    ...
                    f"{_fmt_self_consistency(rm)} | {corr} |"
                )
            lines.append("")

    # Bounded-vs-unbounded drift classification (benchmark-metrics.md research
    # #6): per mode, plot return-error vs duration_s (a path-length proxy
    # under benchmark-spec.md #5's fairness rule) across every recorded run.
    by_mode: dict[str, list[tuple[float, float]]] = {}
    for _, m in entries:
        if m.start_end_closure_error_m is not None:
            by_mode.setdefault(m.mode, []).append((m.duration_s, m.start_end_closure_error_m))
    modes_with_verdicts = {
        mode: classify_bounded_vs_unbounded(points) for mode, points in sorted(by_mode.items())
    }
    if modes_with_verdicts:
        lines += ["## Bounded vs. unbounded drift (by mode)", ""]
        lines.append(
            "Heuristic only (ratio-of-extremes across recorded runs, not a regression) -- see "
            "classify_bounded_vs_unbounded() in tool_benchmark.py."
        )
        lines.append("")
        for mode, verdict in modes_with_verdicts.items():
...
def run_benchmark(
    *,
    ...
    print(
        f"benchmark: loop-closure {_fmt_loop_closure(metrics)} | "
        f"self-consistency {_fmt_self_consistency(metrics)} | "
        f"corrections {metrics.corrections_accepted} acc / {metrics.corrections_rejected} rej"
        + (
            f" | return error {_fmt_start_end(metrics)}"
