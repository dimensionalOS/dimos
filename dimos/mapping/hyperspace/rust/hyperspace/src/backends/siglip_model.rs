...
//! SigLIP / SigLIP2 model, vendored from candle-transformers 0.11.0 (Apache-2.0 / MIT)
//! with two changes: the vision tower can return per-token features, and the
//! attention-pooling head can be applied to every patch token on its own
//! (MaskCLIP-style), which is what makes patch embeddings text-aligned.
...
impl VisionTransformer {
    /// Post-layernorm token features, `[batch, patches, hidden]`.
    pub fn tokens(&self, xs: &Tensor) -> Result<Tensor> {
        let xs = xs.apply(&self.embeddings)?;
        let xs = self.encoder.forward(&xs, None)?;
        xs.apply(&self.post_layernorm)
    }

    /// Pooled embedding `[batch, hidden]` and per-patch pooled embeddings
    /// `[batch, patches, hidden]`: every patch token run through the attention
    /// pooling head as its own length-1 sequence.
    pub fn pooled_and_per_patch(&self, xs: &Tensor) -> Result<(Tensor, Tensor)> {
        let tokens = self.tokens(xs)?;
        let head = self
            .head
            .as_ref()
            .expect("vision head required for pooling");
        let (batch, patches, hidden) = tokens.dims3()?;
        let pooled = tokens.apply(head)?;
        let per_patch = tokens
            .reshape((batch * patches, 1, hidden))?
            .apply(head)?
            .reshape((batch, patches, hidden))?;
        Ok((pooled, per_patch))
    }
}
