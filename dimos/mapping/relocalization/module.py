...
# Jump guard, TRACKING only: largest per-second step from the previous accepted fix.
# T is the world->map alignment, so between accepts it moves only by the drift the fix
# corrects; this refuses the mirror-flipped fix that lands a room over. Floored at one
# second's worth below so back-to-back priors keep a real budget.
...
# Keys that moved onto the prior entries. extra="forbid" already raises on them, but
# the redirect message says where the knob went instead of "extra inputs not permitted".
_MOVED_TO_PRIORS = {
    "reloc_interval_s": (
...
class Config(ModuleConfig):
    ...
    # Operator -o override for the fiducial marker map (a dotted -o can't index the
    # priors list); wins over the blueprint entry in _start_fiducial_prior. None -> use the entry's own.
...
class RelocalizationModule(Module):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # Latest global_map, so a completed tag burst is judged the instant it lands. Sound
        # because the stream ACCUMULATES in the world frame: the previous cloud scores wall
    ...
    def start(self) -> None:
        ...
        # No throttle here: the priors own the cadence. EVERY frame reaches _on_local_map,
        # which asks each prior whether it wants a fire. Trigger accounting runs on the
        # backpressure worker thread only -- a frame backpressure coalesces away never
        # consumes a trigger, so a burst edge cannot be dropped by a slow solve.
    ...
    def _start_fiducial_prior(self) -> None:
        ...
        if not marker_map_file:
            ...
            return
        # A bare name gets the .json default; a name that already states its format keeps it
        # (resolve_named_path would otherwise look up "<survey>.yaml.json", which nothing writes).
    ...
    def _maybe_log_wall_skip(self, n_pts: int, reason: str) -> None:
        ...
        Shares the RANSAC-skip throttle timer -- both are 'cloud too sparse this fire'."""
        now = time.monotonic()
        if now - self._last_skip_log > SKIP_LOG_INTERVAL_S:
            logger.warning(
    ...
    def _try_relocalize(self, msg: PointCloud2, priors: list[RelocPrior]) -> Transform | None:
        ...
        # Plain relocalize() is reserved for the lidar-only MODULE, where the SOLVE is the
        # pre-prior path bit-for-bit and the accept line carries no source=. Once a tag prior
        # is configured every fire goes through the judge instead, so both sources are counted.
        ...
        try:
            ...
            # cycle judges an empty pool. The winner already published it -- benign no-op.
            logger.debug("relocalize: no candidates this cycle")
            return None
        except InsufficientWallEvidenceError as e:
            # Too few wall points to judge the pool (a sparse acquisition cloud a tag burst
            # is allowed to fire on): drop the unjudged fix -- a pose scored against <100
            # walls isn't trustworthy, and the tag is re-seen as the robot moves into structure.
            self._maybe_log_wall_skip(len(msg), str(e))
            return None
        except NoUprightCandidateError:
            # Every candidate tilted past the gravity gate: a real rejection, so the fix is
            # consumed (re-judging a tilted pose only re-rejects it). Throttled warn, shares the skip window.
            now = time.monotonic()
            if now - self._last_skip_log > SKIP_LOG_INTERVAL_S:
        ...
        n_pts = len(msg)
        # source= emission: ACCEPT names the prior that WON (omitted on the single-source
        # path, which the parsers read as ransac); REJECT names the prior whose BAR refused
        # the fix, so --eval can bucket rejects by source instead of "unknown".
        source_kw: dict[str, str] = {} if winning_source is None else {"source": winning_source}

        # The winner is gated on ITS OWN source's bar. winning_source is None only on the
        # plain (ransac-only) path -> resolve to the ransac entry.
        ...
        # Jump guard, TRACKING only: a fix stepping further from the previous one than the
        # budgets above allow is a mis-localization, not a drift correction. No previous fix
        # means acquisition, which is never blocked. Both terms are on map_T_world.
        now_s = self._now_fn()
        if self._last_fix_map_T_world is not None:
            dt_s = now_s - self._last_fix_ts_s
            # Floored at one second's worth: two priors fire back to back may disagree by a
            # normal correction, and a budget shrinking with the gap would refuse exactly the
            # cross-source fix the fiducial preset exists to publish.
