...
def replay(
    ...
    # Needs the native ray tracer, which the other tune commands do without.
...
def _print_attempt(attempt: Attempt, recorded: Transform | None) -> None:
    ...
    if recorded is not None:
        dyaw, dist_m = fix_error(attempt.fix, recorded)
        line += f" vs recorded: yaw {dyaw:+.1f}deg t {dist_m:.2f}m"
