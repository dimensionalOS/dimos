...
def test_detection_transports_resolve_at_runtime() -> None:
    ...
    publishes on an auto topic like /detections_2d or /target instead of the
    authored wire topic. Indexing transport_map by that key fails loudly here if
    a transport was authored under the wrong shape."""
    for (name, msg_type), topic in EXPECTED_TOPICS.items():
        transport = unitree_g1_detection.transport_map[(name, msg_type)]
