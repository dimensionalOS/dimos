...
- front_camera -> mid360_link: lidar is 3.2cm back, 12cm up, pitched 44 deg down —
  the physical sensor pose (position + tilt). Named ``mid360_link`` to match the LIO
  sensor frame_id.
- mid360_link -> mid360_gravity: a counter-rotation (inverse of the mount tilt) that
  leaves the position untouched but un-tilts the orientation, so ``mid360_gravity``
  is gravity-aligned. The LIO odometry is gravity-anchored, so the recorded scans
  live in this gravity-aligned frame at the physical sensor position — keeping the
  physical mount in ``mid360_link`` preserves the geometry.
...
FRAMES: list[FrameSpec] = [
    ...
    # physical sensor pose: position + the 44 deg downward tilt (matches LIO frame_id)
    ("mid360_link", "front_camera", (-0.032, 0.0, 0.12), (0.0, MID360_PITCH_DOWN, 0.0)),
    # gravity-aligned derived frame: same position, tilt counter-rotated out
    ("mid360_gravity", "mid360_link", (0.0, 0.0, 0.0), (0.0, -MID360_PITCH_DOWN, 0.0)),
