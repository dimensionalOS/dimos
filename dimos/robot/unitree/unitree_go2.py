...
class UnitreeGo2(Robot):
    def __init__(
        ...
        super().__init__(
            ros_control=ros_control, output_dir=output_dir, skill_library=skills
        )
    ...
    def follow_human(
        self, distance: int = 1.5, timeout: float = 20.0, point: Tuple[int, int] = None
    ):
        person_visual_servoing = VisualServoing(
            tracking_stream=self.person_tracking_stream
        )

        logger.warning(f"Following human for {timeout} seconds...")
        start_time = time.time()
        success = person_visual_servoing.start_tracking(
            point=point, desired_distance=distance
        )
    ...
    def navigate_to(
        ...
        """
        object_visual_servoing = VisualServoing(
            tracking_stream=self.object_tracking_stream
        )

        logger.warning(
            f"Navigating to {object_name} with desired distance {distance}m, timeout {timeout} seconds..."
        )
        ...
        while bbox is None and retry_count < max_retries:
            if retry_count > 0:
                logger.info(
                    f"Retry {retry_count}/{max_retries} to get bounding box for {object_name}"
                )
                # Wait a moment before retry to let the camera feed update
                time.sleep(1.0)

            try:
                bbox, object_size = get_bbox_from_qwen(
                    self.video_stream_ros, object_name=object_name
                )
        ...
        if bbox is None:
            logger.error(
                f"Failed to get bounding box for {object_name} after {max_retries} attempts"
            )
            return False
