...
Frames (see FDEIssues/alfred-urdf/DECISION_cad_to_urdf_path.md):
  ...
  openarm_{left,right}_base_link  vendor v2.0 flange frames at (0.140, ±0.092, 0.987 + lift).
...
ALFRED_URDF = ALFRED_DESCRIPTION_ROOT / "urdf" / "alfred_v1.urdf"
ALFRED_MODEL = RobotModel.from_file(
    ALFRED_URDF,
    package_paths={
...
def alfred_urdf_joints() -> list[str]:
    return [ALFRED_LIFT_URDF_JOINT, *openarm_urdf_joints("left"), *openarm_urdf_joints("right")]


def alfred_sim_model_config(name: str = "alfred") -> RobotModelConfig:
    """One 15-joint planning robot: lift + both arms, so collision exclusions can span them."""
    return RobotModelConfig(
        name=name,
        model=ALFRED_MODEL,
        base_pose=base_pose(),
        joint_names=alfred_urdf_joints(),
        base_link="base_link",
        planning_groups=[
        ...
        home_joints=[0.0] * (1 + 2 * OPENARM_DOF),
    )
