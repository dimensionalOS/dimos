...
One module owning every pillar-specific detail: the serial port, firmware
commands, framing, units, limits, homing state and command pacing. The control
coordinator reaches it through the generic ``transport_lcm`` whole-body adapter
using one linear joint, ``pillar/lift``, expressed in metres.

The wire protocol is the ``pillar_driver`` host interface. Every line the board
emits falls in exactly one bucket, which is what lets the reader be a
...
* Motion commands preempt each other and never answer ``busy``, so a new target
  is sent straight away rather than queued behind an active move.
...
class PillarConnection(Module):
    """Alfred's pillar, bridged through generic whole-body streams."""
    ...
    def _connect(self) -> None:
        ...
        # Only the running module needs the SDK; importing it at module scope
        # would make every blueprint that composes Alfred unimportable without
        # the alfred extra.
        try:
            import serial
        ...
        with self._state_lock:
            ...
            self._ready_at = now + self.config.boot_wait_s
            self._line_buffer.clear()
            self._pending_replies.clear()
            self._stop_event.clear()
            self._state_changed.notify_all()

        self._reader_thread = Thread(target=self._run, name="alfred-pillar-serial", daemon=True)
    ...
    def _disconnect(self) -> None:
        ...
        if motion_active:
            try:
                ...
                logger.exception("Failed to request pillar stop during shutdown")
                stopped = False
            if stopped and not self._wait_for_motion_end(self.config.shutdown_stop_timeout_s):
                logger.error(
                    "Pillar did not settle after its ramped stop before shutdown; "
    ...
    def home(self) -> bool:
        ...
        with self._state_lock:
            ...
            self._pending_target_m = None
            self._active_target_m = None
            self._home_deadline = now + self.config.home_timeout_s
            self._state_changed.notify_all()
    ...
    def stop_motion(self) -> bool:
        ...
        with self._state_lock:
            ...
            if self._motion_active:
                ...
                # deadline as well.
                self._motion_deadline = now + self.config.move_timeout_s
            self._state_changed.notify_all()
    ...
    def get_status(self) -> dict[str, Any]:
        """Current connection, homing, position, and fault status."""
        with self._state_lock:
            return {
    ...
    def _on_event(self, line: str) -> None:
        ...
        with self._state_lock:
            ...
            if code == "ready":
                ...
                self._motion_deadline = math.inf
                self._phase = PillarPhase.FAULT
                fault = self._fault = "homing failed; check switch, wiring and direction"
                ...
                self._motion_deadline = math.inf
                self._phase = PillarPhase.FAULT
                fault = self._fault = "homing aborted; home is required"
            elif code == "move_done":
                self._motion_active = False
                self._stop_requested = False
                self._active_target_m = None
                ...
                self._motion_deadline = math.inf
                self._phase = PillarPhase.FAULT
                fault = self._fault = "top limit hit; home is required"
                ...
                self._motion_deadline = math.inf
                self._phase = PillarPhase.FAULT
                fault = self._fault = "position reference lost; home is required"
            elif code == "pos_drift":
                # Homing disagreed with the stored position by more than the
                # firmware's tolerance. Homing still completes; a slipping brake
                # is the reason worth chasing.
        ...
        if fault is not None:
            logger.error(f"Alfred pillar fault: {fault}")
    ...
    def _begin_session(self, now: float) -> None:
        with self._state_lock:
            ...
            if self._phase is PillarPhase.UNKNOWN and self._fault is None:
                self._phase = PillarPhase.IDLE
            rate = self.config.telemetry_rate_hz
            speed = self.config.speed_mm_s
            accel = self.config.accel_mm_s2
            self._state_changed.notify_all()

        # Fire-and-forget: this runs on the reader thread, which is the thread
    ...
    def _dispatch_pending_target(self) -> None:
        ...
        with self._state_lock:
            ...
            self._next_command_at = now + self.config.min_command_interval_s
            self._motion_active = True
            self._motion_deadline = now + self.config.move_timeout_s
    ...
    def _command(
        ...
        *,
        ...
        if pending is None:
            return None
        timeout = self.config.reply_timeout_s if timeout_s is None else timeout_s
