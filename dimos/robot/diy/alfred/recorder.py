...
`wheel_odometry` is the drive train's own integrated pose, polled off the
controller by `AlfredHighLevel`. It is recorded next to Point-LIO's `odometry`
rather than fused into it, so a consumer can pick which one to trust.
"""
...
class AlfredRecorderConfig(RecorderConfig):
    db_path: str | Path = Field(
        default_factory=lambda: f"alfred_realsense_{datetime.now():%Y-%m-%d_%H-%M-%S}.db"
    )
