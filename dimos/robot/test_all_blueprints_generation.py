...
def _is_production_module_file(file_path: Path, root: Path) -> bool:
    ...
    return not (
        ...
        or "/testing/" in rel
        # Isolated Python implementations are private subprocess details. Only
...
def test_isolated_python_runtime_is_not_a_production_module(tmp_path: Path) -> None:
    runtime = tmp_path / "feature" / "python" / "package" / "runtime.py"

    assert _is_production_module_file(runtime, tmp_path) is False


def test_isolated_python_runtime_tree_is_not_scanned(tmp_path: Path) -> None:
...
def _get_all_python_files(root: Path) -> Generator[Path, None, None]:
    """Yield host source files without entering isolated Python runtime projects."""
