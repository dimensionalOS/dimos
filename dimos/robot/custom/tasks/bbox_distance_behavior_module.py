...
class BBoxDistanceBehaviorConfig(ModuleConfig):  # 定义 bbox 距离行为模块配置
    ...
    depth_bbox_padding_px: float = 12.0  # 距离估计时对 bbox 做像素扩张，提升稀疏点云命中率
    depth_bbox_max_padding_px: float = 72.0  # 距离估计失败时允许继续扩张 bbox 的最大像素
    depth_bbox_padding_step_px: float = 12.0  # 逐级扩张 bbox 的步长像素
...
class BBoxDistanceBehaviorModule(Module):  # 定义距离行为模块，只消费 selected bbox、lidar 和 camera_info
    ...
    def _compute_next_twist(self) -> Twist | None:  # 计算当前周期应该发布的速度
        ...
        if distance is None:  # 如果当前点云无法在 bbox 内给出有效距离
            self._set_block_reason(f"no_depth_for_target:{self._detection_id(detection)!r}")
    ...
    def _estimate_bbox_distance(  # 定义 bbox 内点云距离估计函数
        ...
        base_padding = max(float(self.config.depth_bbox_padding_px), 0.0)
        max_padding = max(base_padding, float(self.config.depth_bbox_max_padding_px))
        step_padding = max(float(self.config.depth_bbox_padding_step_px), 1.0)
        ...
        while padding <= max_padding + 1e-6:
            px1 = x1 - padding
            py1 = y1 - padding
            px2 = x2 + padding
            py2 = y2 + padding
            inside = (u >= px1) & (u <= px2) & (v >= py1) & (v <= py2)  # 计算落在当前扩张 bbox 内的点
