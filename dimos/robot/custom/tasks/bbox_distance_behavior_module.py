...
from dimos.navigation.replanning_a_star.module_spec import ReplanningAStarPlannerSpec  # 导入规划器 Spec，用于 bbox 任务启动时取消 A* 导航
...
class BBoxDistanceBehaviorModule(Module):  # 定义距离行为模块，只消费 selected bbox、lidar 和 camera_info
    ...
    _planner: ReplanningAStarPlannerSpec | None  # 可选：A* 规划器，存在时用于 bbox 任务启动时取消当前导航
    ...
    def __init__(self, **kwargs: Any) -> None:  # 定义构造函数，接收框架传入的配置参数
        ...
        self._planner: ReplanningAStarPlannerSpec | None = None  # 可选规划器引用，由 blueprint 在 build 时注入
