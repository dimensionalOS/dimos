...
import os  # 导入系统环境变量工具，用来修正 macOS 的 pygame 显示驱动
import sys  # 导入系统信息工具，用来判断当前是不是 macOS
...
from dimos.robot.unitree.go2.blueprints.basic.unitree_go2_coordinator import (  # 导入现有 Go2 控制协调蓝图
    unitree_go2_coordinator,  # 复用 GO2Connection + ControlCoordinator 控制链路
)  # 结束多行 import
from dimos.robot.unitree.keyboard_teleop import KeyboardTeleop  # 导入现有 WASD 键盘控制模块
from dimos.utils.logging_config import setup_logger  # 导入日志初始化函数

if sys.platform == "darwin":  # 如果当前运行环境是 macOS
    os.environ["SDL_VIDEODRIVER"] = "cocoa"  # 用 macOS 原生窗口驱动，覆盖原模块里的 x11 设置
...
# 复用现有 Go2 协调控制蓝图，再额外加上 WASD 键盘控制和一次性启动自检发布者。
unitree_go2_startup_self_check = autoconnect(  # 定义 CLI 可运行的组合蓝图
    unitree_go2_coordinator,  # 第一部分：现有 Go2 + ControlCoordinator 控制链路
    KeyboardTeleop.blueprint(publish_only_when_active=True),  # 第二部分：WASD 按下时才发布速度
    Go2StartupSelfCheck.blueprint(),  # 第三部分：新增的启动自检模块
)  # 结束蓝图组合
...
__all__ = [  # 声明这个文件希望对外暴露的名字
    ...
    "unitree_go2_startup_self_check",  # 暴露自检蓝图变量
]  # 结束 __all__ 列表
