...
class Go2StartupSelfCheck(Module):  # 定义一个 DimOS 模块，用来发布启动自检速度命令
    ...
    def _run_delayed_self_check(self) -> None:  # 定义延迟兜底启动逻辑
        ...
        if self._stop_event.wait(
            self.config.startup_delay_sec
        ):  # 等待配置的秒数，期间如果收到停止就返回 True
            return  # 如果停止事件已经触发，就不再启动自检

        self._start_self_check(
            "startup delay elapsed"
        )  # 等待结束后，如果还没启动过，就兜底启动自检
    ...
    def _publish_for_duration(
        self, speed_mps: float, duration_sec: float
    ) -> None:  # 按固定时间持续发布速度
        ...
        while (
            not self._stop_event.is_set() and time.monotonic() < end_time
        ):  # 未停止且未超时时继续发布
