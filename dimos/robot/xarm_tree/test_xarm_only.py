...
"""Test script for XArm only (no mobile base)."""
...
logger = setup_logger("test_xarm_only")
...
class TestInterface:
    """Test interface for XArm robot only."""

    def __init__(self, robot: XArmRobot):
        self.robot = robot
        self.lcm = LCM()
...
def main():
    ...
    print("=" * 60)
    print("XArm Robot Test (No Mobile Base)")
    print("=" * 60)
    print("This test expects:")
    print("  1. XArmRobot running")
    print("  2. ZED camera publishing to LCM topics")
    print("=" * 60)
    ...
    logger.info("Moving arm to observe position...")
    if robot.xarm:
        robot.xarm.goto_observe()
        logger.info("Arm moved to observe position")
    else:
        logger.error("Failed to move arm to observe position")
    ...
    cv2.namedWindow("XArm Camera")
    cv2.setMouseCallback("XArm Camera", mouse_callback)
    ...
    print("  '2' - Move EE down (Z-)")
    print("  '8' - Move EE up (Z+)")
    print("  '4' - Move EE back (X-)")
    print("  '6' - Move EE forward (X+)")
    print("  '7' - Pitch down (Y-)")
    print("  '9' - Pitch up (Y+)")
    print("\nPICK AND PLACE MODE:")
    ...
    try:
        while interface._running:
            ...
            if interface.latest_camera is not None:
                ...
                cv2.imshow("XArm Camera", display_image)
            ...
            if key != 255:  # Key was pressed
                if key == ord("q"):
                    ...
                    # Pass all other keys to manipulation module keyboard handler
