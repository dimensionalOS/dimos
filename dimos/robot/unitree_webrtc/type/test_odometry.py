...
def _replay_iter(autocast=None) -> Iterable[RawOdometryMessage]:
    return SensorReplay(name=_REPLAY_NAME, autocast=autocast).iterate()


def test_dataset_size() -> None:
    """Ensure the replay contains the expected number of messages."""
    count = sum(1 for _ in _replay_iter())
...
def test_last_yaw_value() -> None:
    ...
    last_msg: Optional[RawOdometryMessage] = None
    for msg in _replay_iter():
        last_msg = msg  # streaming – retain only the most recent
...
def test_total_rotation_from_odometry() -> None:
    ...
    for odom in _replay_iter(Odometry.from_msg):
        yaw = odom.rot.z
        if prev_yaw is not None:
            diff = yaw - prev_yaw
