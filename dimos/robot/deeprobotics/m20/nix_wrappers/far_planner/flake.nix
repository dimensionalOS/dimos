{
  description = "Wrapper flake for dimensionalOS/dimos-module-far-planner with M20 NOS kernel 5.10 unpackPhase workaround";

  # See sibling local_planner/flake.nix for full workaround rationale.

  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
    mod.url = "github:dimensionalOS/dimos-module-far-planner/v0.5.0";
  };
  ...
  outputs = { self, nixpkgs, mod, ... }:
    ...
    in {
      packages.${system}.default = base.overrideAttrs (old: {
        ...
        postInstall = ''
          ${old.postInstall or ""}
          if [ -x "$out/bin/far_planner_native" ] && [ ! -e "$out/bin/far_planner" ]; then
            ln -s far_planner_native "$out/bin/far_planner"
          fi
        '';
      });
    };
