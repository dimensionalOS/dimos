...
from dimos.robot.cli.shell import _format_description, _shell_namespace


def test_shell_rejects_non_interactive_execution(mocker):
    mocker.patch("dimos.robot.cli.shell._is_interactive_terminal", return_value=False)
    connect = mocker.patch("dimos.robot.cli.shell.Dimos.connect")
...
def test_shell_starts_ipython_with_debug_namespace_and_disconnects(mocker):
    ...
    connect = mocker.patch("dimos.robot.cli.shell.Dimos.connect", return_value=app)
    mocker.patch("dimos.robot.cli.shell.get_most_recent", return_value=None)
    start_ipython = mocker.patch("dimos.robot.cli.shell.start_ipython")
...
def test_shell_disconnects_when_ipython_reports_connection_loss(mocker):
    ...
    mocker.patch("dimos.robot.cli.shell.get_most_recent", return_value=None)
    mocker.patch(
        "dimos.robot.cli.shell.start_ipython",
