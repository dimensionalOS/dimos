...
R1PRO_TOPICS: dict[str, Topic] = {
    ...
    # The head is a plain RGB stereo pair — Galaxea's spec calls it "1x pure
    # binocular RGB camera" and there is no head depth topic on the robot — so
    # both eyes' intrinsics have to reach the stereo matcher separately. The
    # robot does publish these, on /calib/head_{left,right}/camera_info.
    "head_left_info": Topic("/head_left_info", CameraInfo, latest_wins=True),
    "head_right_info": Topic("/head_right_info", CameraInfo, latest_wins=True),
