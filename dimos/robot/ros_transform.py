...
class ROSTransformAbility:
    ...
    def transform_path(
        ...
        for point in path:
            transformed_point = self.transform_point(
                point, source_frame, target_frame, timeout
            )
    ...
    def transform_rot(
        ...
        try:
            ...
            logger.error(
                f"Transform rotation from {source_frame} to {target_frame} failed: {e}"
            )
            return None
    ...
    def transform_pose(
        ...
        """
        # Transform position
        transformed_position = self.transform_point(
            position, source_frame, target_frame, timeout
        )

        # Transform rotation
        transformed_rotation = self.transform_rot(
            rotation, source_frame, target_frame, timeout
        )
