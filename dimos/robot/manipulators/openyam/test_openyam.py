...
def test_openyam_model_config_has_expected_links_and_canonical_joints() -> None:
    ...
    assert list(config.model.load().package_paths) == list(OPENYAM_PACKAGE_PATHS)
...
def test_openyam_mock_hardware_has_gripper() -> None:
    ...
    assert hardware.joints == [
        *(f"yam_joint{i}" for i in range(1, OPENYAM_DOF + 1)),
        "arm/gripper",
    ]
...
def test_openyam_planner_blueprint_preserves_model_config() -> None:
    ...
    assert task.joint_names == [*config.joint_names, "arm/gripper"]


def test_openyam_coordinator_blueprint_routes_arm_and_gripper_joints() -> None:
