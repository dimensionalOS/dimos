...
def test_backend_and_camera_devices_are_normal_module_cli_config():
    ...
    assert parsed.module_kwargs("policy")["backend"] == "lerobot"
    assert parsed.module_kwargs("policy")["policy_joint_names"] is None
    assert (
        parsed.module_kwargs("policy")["image_mapping"]["overhead_image"]
        == "observation.images.overhead"
    )
    assert parsed.module_kwargs("left_wrist")["serial_number"] == "left-camera"
    assert parsed.module_kwargs("right_wrist")["serial_number"] == "right-camera"
