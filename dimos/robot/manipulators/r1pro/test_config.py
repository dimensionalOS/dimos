...
from dimos.robot.manipulators.r1pro.blueprints.basic import r1pro_planner_coordinator
from dimos.robot.manipulators.r1pro.config import (
    R1PRO_DESCRIPTION_REF,
    R1PRO_DESCRIPTION_SOURCE,
    R1PRO_DESCRIPTION_URL,
    R1PRO_JOINTS,
...
def test_r1pro_uses_pinned_official_2026_description() -> None:
    assert R1PRO_DESCRIPTION_URL == "https://github.com/userguide-galaxea/URDF"
    assert R1PRO_DESCRIPTION_REF == "2e5d31e1784481a34d178006c0d0e18e0a84a82a"
    assert R1PRO_DESCRIPTION_SOURCE.url == R1PRO_DESCRIPTION_URL
    assert R1PRO_DESCRIPTION_SOURCE.ref == R1PRO_DESCRIPTION_REF
...
def test_r1pro_config_exposes_disjoint_mobile_bimanual_groups() -> None:
    ...
    assert config.joint_names == R1PRO_JOINTS
    ...
    assert set(grouped_joints) == set(R1PRO_JOINTS)


def test_r1pro_groups_compose_arm_pose_targets_with_mobile_auxiliaries() -> None:
    registry = PlanningGroupRegistry([make_r1pro_model_config()])
    ...
    assert set(bimanual_mobile.joint_names) == set(R1PRO_JOINTS)


def test_r1pro_fake_hardware_covers_every_planned_coordinate() -> None:
    hardware = r1pro_fake_hardware()

    assert hardware.hardware_id == "r1pro"
    assert hardware.adapter_type == "mock"
    assert hardware.joints == R1PRO_JOINTS
...
def test_r1pro_blueprint_wires_viser_planner_to_fake_hardware() -> None:
    ...
    assert coordinator.hardware[0].adapter_type == "mock"
    assert coordinator.hardware[0].joints == R1PRO_JOINTS
    assert coordinator.tasks[0].joint_names == R1PRO_JOINTS
...
def test_r1pro_materialized_model_matches_configured_coordinates() -> None:
    ...
    assert {joint.name for joint in model.joints if joint.type != "fixed"} == set(R1PRO_JOINTS)
...
def test_r1pro_arm_ik_leaves_the_singular_zero_pose() -> None:
    ...
    groups = PlanningGroupRegistry([config])
