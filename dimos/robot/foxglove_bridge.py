...
class FoxgloveBridge(Module):
    ...
    def __init__(self, *args, shm_channels=None, jpeg_shm_channels=None, **kwargs) -> None:  # type: ignore[no-untyped-def]
        ...
        print(
            f"[FoxgloveBridge.__init__] Called with shm_channels={shm_channels}, jpeg_shm_channels={jpeg_shm_channels}",
            file=sys.stderr,
            flush=True,
        )
        print(
            f"[FoxgloveBridge.__init__] kwargs keys: {list(kwargs.keys())}",
            file=sys.stderr,
            flush=True,
        )
        ...
        logger.info(
            f"[FoxgloveBridge.__init__] Initializing with shm_channels={self.shm_channels}, jpeg_shm_channels={self.jpeg_shm_channels}"
        )
        ...
        def run_bridge() -> None:
            ...
            try:
                ...
                if self.jpeg_shm_channels:
                    logger.warning(
                        f"[FoxgloveBridge] jpeg_shm_channels not supported by LCMFoxgloveBridge, ignoring: {self.jpeg_shm_channels}"
                    )
                ...
                if "jpeg_shm_channels" in bridge_kwargs:
                    logger.error(
                        "[FoxgloveBridge] CRITICAL: jpeg_shm_channels found in bridge_kwargs! Removing it..."
                    )
                    del bridge_kwargs["jpeg_shm_channels"]

                logger.info(
                    f"[FoxgloveBridge] Creating bridge with kwargs: {list(bridge_kwargs.keys())}"
                )
                ...
                try:
                    ...
                    logger.info(
                        "[FoxgloveBridge] Bridge initialized successfully, starting server..."
                    )
                    logger.info(
                        "[FoxgloveBridge] Bridge will automatically forward all LCM topics to Foxglove"
                    )
...
"""
        ...
        print(
            f"[FoxgloveBridge] Bridge thread started (daemon={self._thread.daemon})",
            file=sys.stderr,
            flush=True,
        )
