...
class StreamedMotionMerger:
    ...
    def merge(self, fields: dict[str, NDArray[Any]], current_playback_frame: int) -> MergeResult:
        ...
        if merge_dst > 0 and self._motion is not None and self._motion.timesteps > 0:
            ...
            if ov_start < ov_end:
                ...
                if n > 0:
                    ...
                    if (
                        new.smpl_joints is not None
                        and old.smpl_joints is not None
                        and new.smpl_pose is not None
                        and old.smpl_pose is not None
                    ):
        ...
        if (
            new.smpl_joints is not None
            and new.smpl_pose is not None
            and smpl_joints is not None
            and smpl_pose is not None
        ):
