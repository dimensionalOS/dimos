...
def test_ignores_commands_addressed_to_other_tasks(task: G1DualArmIKTask) -> None:
    # The card delivers the port ungated; foreign task names are dropped
    # without storing a target.
    assert not task.on_cartesian_command(_pose(0.3, 0.2, 0.2, "other_task/left"), t_now=1.0)
    assert not task.on_cartesian_command(_pose(0.3, 0.2, 0.2, "left"), t_now=1.0)
    task.on_buttons(_buttons(1.0, 1.0))
    assert task.compute(_state_at(1.0)) is None


def test_estop_drops_held_pose_and_goes_inert(task: G1DualArmIKTask) -> None:
    task.on_buttons(_buttons(1.0, 1.0))
    ...
    assert task.compute(_state_at(1.0)) is not None

    task.set_estop(True)
    assert not task.is_active()
    assert task.compute(_state_at(1.1)) is None
    # Clearing the latch does not resurrect the dropped pose.
    task.set_estop(False)
    assert task.compute(_state_at(1.2)) is None
