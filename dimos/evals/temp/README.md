...
| File | What it is |
|---|---|
| `split.py` | train / holdout / spare slices over generated rows; the three sliced pointcloud suites import it |
...
tag), the `split.assign(...)` call in the three sliced suites, and the `train`
tag in the four hand-authored ones — after that the suites run every row, as
they did before.
