...
class RecordingFramePreprocessor:
    ...
    def load_topdown(self, frame_index: int) -> TopDownFrame:
        ...
        if self._odom is None or self._global_lidar_map is None:
            raise FrameGeometryUnavailableError("recording has no 'odom' stream")
    ...
    def _build_global_lidar_map(self, resolution: float = 0.05) -> GlobalLidarMap:
        ...
        if self._odom is None or self._lidar is None:
            raise FrameGeometryUnavailableError("global LiDAR map requires odometry and LiDAR")
        poses = [
            observation.data
        ...
        if not poses:
            raise FrameGeometryUnavailableError("recording has no finite odometry observations")
        ...
        if width * height > 20_000_000:
            raise FrameGeometryUnavailableError("global LiDAR map exceeds the safe grid size")
