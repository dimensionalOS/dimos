...
_DATASET = str(
    ...
    / "Documents/worktrees/agentencode/recording_go2.db"
)
_EVIDENCE_INSTRUCTIONS = (
    "Open the recording file listed in the system prompt and use only its odom PoseStamped "
    "observations. Iterate over every observation in timestamp order and call "
    "obs.data.agent_encode() on each one. Perform all analysis using only the returned "
    "dictionaries. Do not inspect raw pose fields, skip observations, construct a Path, or "
    "create or inspect images. "
)


def _poses() -> Dataset:
    return Dataset(_DATASET, select=(lambda store: store.streams.odom,))
