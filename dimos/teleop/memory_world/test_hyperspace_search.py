...
def test_a_search_that_exits_instead_of_raising_is_recorded_as_failed(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    A first load now gets the same RELOAD_ATTEMPTS a reload does -- a transient failure
    must not be permanent -- so the SystemExit is RECORDED on the first attempt and the
    error latches once those attempts are spent. Both halves matter: recorded, so the
    retries are spaced and counted rather than run every poll; latched, so a genuinely
    wrong memory db stops costing a text tower and says why.
    """
    import threading
    from types import SimpleNamespace

    from dimos.teleop.memory_world.hyperspace_answers import RELOAD_ATTEMPTS, HyperspaceAnswers
    ...
    with (
        ...
        # Caught and recorded, not let out and not yet given up on.
        assert module._failed_stamp == (1, 1.0) and module._failed_count == 1
        assert module._hyperspace_error is None

        for _ in range(RELOAD_ATTEMPTS - 1):
            assert module._load_hyperspace() is False
    assert module._failed_count == RELOAD_ATTEMPTS
