...
# A body frame (x forward, z up) seen as a camera optical frame (z forward, y down): the
# standard ROS optical rotation. Used only for recordings that carry no tf tree and stamp
# body poses on their images.
OPTICAL_FROM_BODY: np.ndarray


def body_camera_pose(obs: object) -> np.ndarray | None:
    """world_T_optical from the body pose stamped on an observation, or None.

    The no-tf fallback, here rather than in the module because the indexer SUBPROCESS
    needs the same one: it built its poses from tf alone and died with `KeyError: 'tf'`
    on exactly the recordings this exists for.
    """
