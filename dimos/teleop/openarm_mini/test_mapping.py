...
def test_mapping_uses_direct_arm_joint_assignment_and_follower_names() -> None:
    ...
    assert list(command.positions_by_joint) == [f"left_arm/joint{i}" for i in range(1, 8)]
    assert command.positions_by_joint["left_arm/joint1"] == pytest.approx(0.1)
    assert command.positions_by_joint["left_arm/joint6"] == pytest.approx(0.6)
    assert command.positions_by_joint["left_arm/joint7"] == pytest.approx(0.7)
...
def test_combined_command_uses_openarm_follower_joint_names() -> None:
    ...
    assert joint_state.name == [
        *[f"left_arm/joint{i}" for i in range(1, 8)],
        *[f"right_arm/joint{i}" for i in range(1, 8)],
    ]
...
def test_follower_joint_limits_clamp_sender_side() -> None:
    ...
    assert left.positions_by_joint["left_arm/joint1"] == pytest.approx(1.3963)
    assert right.positions_by_joint["right_arm/joint1"] == pytest.approx(3.4907)
    assert left.positions_by_joint["left_arm/joint4"] == pytest.approx(0.0)
...
def test_gripper_reading_is_not_required() -> None:
    ...
    assert list(command.positions_by_joint) == [f"right_arm/joint{i}" for i in range(1, 8)]
