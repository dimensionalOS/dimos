...
class PointcloudFiltering:
    ...
    def __init__(
        ...
        if radius_filtering_min_neighbors < 1:
            raise ValueError(f"radius_filtering_min_neighbors must be >= 1, got {radius_filtering_min_neighbors}")
        ...
        if cuboid_method not in ['minimal', 'oriented', 'axis_aligned']:
            raise ValueError(f"cuboid_method must be 'minimal', 'oriented', or 'axis_aligned', got {cuboid_method}")
    ...
    def _validate_inputs(self, color_img: np.ndarray, depth_img: np.ndarray, segmentation_result: SegmentationType):
    ...
    def _apply_color_mask(self, pcd: o3d.geometry.PointCloud, rgb_color: np.ndarray) -> o3d.geometry.PointCloud:
    ...
    def process_images(self, color_img: np.ndarray, depth_img: np.ndarray, segmentation_result: SegmentationType) -> Dict:
        """
...
def main():
    ...
    try:
        ...
        print(f"  Objects processed: {stats['total_objects_processed']}/{stats['total_input_objects']}")
        print(f"  Total points: {stats['total_points_processed']}")
        
        # Print per-object stats
        for i, obj in enumerate(results['objects']):
            print(f"  Object {i+1} (ID: {obj['object_id']}): {len(np.asarray(obj['point_cloud'].points))} points")
