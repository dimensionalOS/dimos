...
from dimos_generated.geometry_msgs.msg import PointStamped, PoseArray, PoseStamped, Transform, Twist
from dimos_generated.nav_msgs.msg import OccupancyGrid, Odometry, Path
from dimos_generated.sensor_msgs.msg import Image, Imu, JointState, PointCloud2
...
from dimos.web.cdr_codec import encode_cdr_v1, export_schema
...
def test_image_has_no_default_encoding() -> None:
    ...
    with pytest.raises(
        ValueError, match=r"sensor_msgs/msg/Image has no default web encoding.*jpeg\.v1"
    ):
        Channel("snapshot", Image)


def test_channel_default_encoding_by_type() -> None:
    assert Channel("pose", PoseStamped).encoding == "geometry_msgs/msg/PoseStamped.cdr.v1"
    # Transform and TFMessage are distinct generated messages.
    assert Channel("tf", Transform).encoding == "geometry_msgs/msg/Transform.cdr.v1"
...
def test_cdr_channel_blueprint() -> None:
    blueprint = cockpit(channels=[Channel("pose", PoseStamped, max_hz=20.0)])
    (atom,) = blueprint.blueprints
    manifest = atom.kwargs["manifest"]
    (channel,) = manifest["channels"]
    schema = export_schema(PoseStamped)
    assert channel["encoding"] == "geometry_msgs/msg/PoseStamped.cdr.v1"
    assert channel["params"] == {"cdr": schema}
    ...
    restored = pickle.loads(pickle.dumps(blueprint))
    assert restored.blueprints[0].kwargs["channels"][0].encoder is encode_cdr_v1


def test_generated_class_channel_uses_its_canonical_name() -> None:
    # Default and explicit encodings both use the generated qualified name.
    markers = MarkerArray
    blueprint = cockpit(
        channels=[
            Channel("markers", markers),
            Channel("markers_too", markers, encoding="visualization_msgs/msg/MarkerArray.cdr.v1"),
        ]
    )
    (atom,) = blueprint.blueprints
    for wire in atom.kwargs["manifest"]["channels"]:
        assert wire["encoding"] == "visualization_msgs/msg/MarkerArray.cdr.v1"
        assert wire["params"]["cdr"]["type"] == "visualization_msgs/msg/MarkerArray"
    assert all(spec.encoder is encode_cdr_v1 for spec in atom.kwargs["channels"])


def test_cdr_schema_joins_user_params_in_the_request_only() -> None:
...
def test_all_generated_types_have_default_cdr_encoders() -> None:
...
def _encode_note_cdr(msg: _OpsNote) -> bytes:
    return msg.text.encode()


def test_user_registered_cdr_v1_encoding_wins() -> None:
    # An explicit @web_encoder with an *.cdr.v1 id is its own codec: no schema
    # injected, no reserved params key.
    channel = Channel("ops_note", _OpsNote, encoding="t.ck.cdr.v1", params={"cdr": 1})
    (atom,) = cockpit(channels=[channel]).blueprints
    (wire,) = atom.kwargs["manifest"]["channels"]
    assert wire["params"] == {"cdr": 1}
    (spec,) = atom.kwargs["channels"]
    assert spec.encoder is _encode_note_cdr and spec.encoder_takes_params is False


def test_cdr_channels_fit_the_control_payload_cap() -> None:
    # Every CDR channel adds its schema (150 B to 1.5 KB) to the hello frame.
    channels = [
    ...
    size = len(encode_datagram(hello))
    assert size <= MAX_CONTROL_PAYLOAD_BYTES, f"hello with six CDR channels is {size} B"
