...
"""Zenoh-based video provider module.

This module provides a video frame provider that receives frames from Zenoh
...
class ZenohVideoProvider(AbstractVideoProvider):
    """Video provider that uses a Subject to broadcast frames pushed by Zenoh.

    This class implements a video provider that receives frames from Zenoh and makes them
    ...
    def __init__(
        self, dev_name: str = "zenoh_video", pool_scheduler: Optional[ThreadPoolScheduler] = None
    ):
        """Initialize the Zenoh video provider.
        ...
        self.logger.info("ZenohVideoProvider initialized")
    ...
    def get_stream(self, fps: int = 30) -> Observable:
        """Get video stream - compatibility method that calls capture_video_as_observable.

        Args:
            fps: Frames per second rate limit

        Returns:
            Observable: An observable stream of video frames
        """
        return self.capture_video_as_observable(fps=fps)
