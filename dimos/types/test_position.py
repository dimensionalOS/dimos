...
def test_position_named_args():
    """Test initialization with named arguments."""
    pos = Position(pos=Vector(1.0, 2.0, 3.0), rot=Vector(4.0, 5.0, 6.0))
    
    assert pos.x == 1.0
    assert pos.y == 2.0
    assert pos.z == 3.0
    assert pos.rot.x == 4.0
    assert pos.rot.y == 5.0
    assert pos.rot.z == 6.0
