...
Flattenable = Annotated[
    Literal["dict", "np", "pt", "list"], "Numpy, PyTorch, list, or dict"
]
...
class Sample(BaseModel):
    ...
    def flatten(
        ...
        def flatten_recursive(obj, path=""):
            if isinstance(obj, Sample):
                ...
                if non_numerical == "ignore" and not isinstance(
                    obj, int | float | bool
                ):
                    return
    ...
    def space_for(
        ...
        if isinstance(value, int | float | list | tuple | np.ndarray):
            ...
            try:
                ...
                return spaces.Tuple(
                    [
                        cls.space_for(value[0], max_text_length, info)
    ...
    def unpack(self, to_dicts=False) -> List[Union["Sample", Dict]]:
        ...
        if to_dicts:
            return [
                {key: getattr(self, key)[i] for key in attributes}
    ...
    def space(self) -> spaces.Dict:
        ...
        for key, value in self.dict().items():
            ...
            space_dict[key] = (
                value.space()
                if isinstance(value, Sample)
                else self.space_for(value, info=info)
            )
