...
def bake_scene_mjcf(
    ...
    from dimos.simulation.mujoco.mesh_scene import load_scene_prims
...
def cli_main() -> None:
    """``python -m dimos.simulation.mujoco.usdz_to_mjcf <scene_path> <robot_mjcf> [scale] [--view]``.
    ...
    if len(args) < 2:
        print(
            "usage: python -m dimos.simulation.mujoco.usdz_to_mjcf <scene_path> <robot_mjcf> [scale] [--view]"
        )
        sys.exit(2)
    scene = Path(args[0])
    robot = Path(args[1])
