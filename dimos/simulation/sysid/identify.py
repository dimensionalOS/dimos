...
"""What can this recording actually measure? Sensitivity before fitting.
    ...
    python -m dimos.simulation.sysid.identify REC.mcap --channel accel
...
from dimos.simulation.sysid.score import predicted, residual as channel_residual
...
def nudge(
    ...
    *,
    ...
    The centre is the applied value if present, else the engine default the
    robot declares in ``defaults`` (no early preset carries a contact or
    ...
    a guess.
    """
    value = float(values.get(name, (defaults or {}).get(name, float("nan"))))
...
def _variants(
    ...
    for name in names:
        for sign in (+1, -1):
            out.append(nudge(values, name, knobs[name], frac, sign, defaults=defaults))
    return out
...
def jacobian(
    ...
    *,
    ...
    """
    knobs = backend.knobs()
    # The default set is every knob not in ``exclude``: a robot excludes its
    # solver keys, integer/categorical model options on which a central
    # difference measures solver convergence (or, after rounding, nothing at
    # all), not the plant. Ask for them by name to get exactly that measurement.
    names = list(params) if params is not None else [n for n in knobs if n not in exclude]
    ...
    if rollouts is not None:
        specs = [
            ...
            for vals in _variants(values, names, knobs, frac, defaults)
        ]
    ...
    for seg_i, seg in enumerate(segments):
        if flat is not None:
            ...
            block = [run(seg_i, seg, v) for v in _variants(values, names, knobs, frac, defaults)]
        base = block[0]
...
def main(
    ...
    *,
    ...
    ap = argparse.ArgumentParser(prog="sysid.identify")
    ...
    rollouts = (
        Rollouts(args.recording, backend, reader=reader, workers=args.workers)
        if args.workers > 1
        else None
    )
    sens = jacobian(
