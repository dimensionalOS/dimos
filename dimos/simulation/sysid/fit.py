...
from dimos.simulation.sysid.recording import RecordingReader, read_declarations
...
def start_values(preset: Preset, engine_defaults: Mapping[str, float]) -> dict[str, float]:
    """The incumbent's complete knob values: the search's start point.

    Early presets predate the contact and solver keys, so the robot's engine
    defaults fill in: this changes no physics, it just gives every knob a
    start value, and the preset a fit SAVES then carries them explicitly.
    """
    return {**engine_defaults, **preset.physics, "actuator_tau": preset.actuator_tau}
...
def judgement(
    ...
    *,
    engine_defaults: Mapping[str, float],
) -> str:
    ...
    baseline = objective.calibrate(merged(base, plan, {}))
    tvals = start_values(target, engine_defaults)
    ...
    for name in sorted(tvals):
        ...
        if not why and name in engine_defaults and tvals[name] == engine_defaults[name]:
            why = "engine default (never overridden)"
...
def main(
    ...
    *,
    ...
    backend_cls: Callable[..., Any],
    load_preset: Callable[[str | None], Preset],
    default_preset: str,
    envelopes: Mapping[str, TorqueEnvelope],
    engine_defaults: Mapping[str, float],
    default_plan: Callable[[Mapping[str, Knob], str | None, Sequence[str] | None], KnobPlan],
) -> None:
    """Loop 1: fit the searched knobs on one or more recordings.

    The robot supplies its reader, engine, preset loader, envelope table,
    engine defaults and its anchoring plan (``default_plan(knobs, robot_json,
    search)``); everything else is the method.
    """
    ap = argparse.ArgumentParser(prog="sysid.fit")
    ap.add_argument(
        "recordings",
        nargs="+",
    ...
    ap.add_argument("--preset", default=default_preset, help="the incumbent the search starts from")
    ...
    backend = backend_cls(envelope=envelopes[args.envelope] if args.envelope else None)
    base = start_values(load_preset(args.preset), engine_defaults)
    search = tuple(args.search.split(",")) if args.search else None
    if args.knobs:
        plan = load_knob_plan(args.knobs, backend.knobs())
    else:
        plan = default_plan(backend.knobs(), args.robot, search)
    ...
    with ExitStack() as stack:
        for idx, (rec, declared) in enumerate(zip(args.recordings, declarations, strict=True)):
            ...
            rollouts = stack.enter_context(
                Rollouts(rec, backend, reader=reader, workers=part_workers)
            )
            st = rollouts.streams
        ...
        if args.judge is not None:
            ...
            print()
            print(judgement(objective, base, plan, target, args, engine_defaults=engine_defaults))
            return
    ...
    if args.held_out:
        held_out_lines = _held_out(args, base, plan, res, weights, backend, reader)
...
def _held_out(
    ...
    ho = Rollouts(args.held_out, backend, reader=reader, workers=args.workers)
    with ho:
        st = ho.streams
        spans = regimes(st, declared)
        t_lo = max(float(st.lt[0]), float(st.ct[0]))
