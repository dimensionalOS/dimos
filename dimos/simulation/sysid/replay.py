...
from dimos.simulation.sysid.recording import RecordingReader, Streams
...
class ReplayResult:
    ...
    dq_real: np.ndarray = dataclasses_field(default_factory=lambda: np.zeros((0, 0)))
...
def score(pred: Prediction, st: Streams) -> ReplayResult:
    ...
    return ReplayResult(
        ...
        tau_real=st.ltau[aidx] if len(st.ltau) else np.zeros((len(pred.at), pred.tau.shape[1])),
        dq_real=st.ldq[idx] if len(st.ldq) else np.zeros((len(pred.t), pred.q.shape[1])),
    )


def ghost_track(st: Streams) -> GhostTrack | None:
...
def replay(
    ...
    *,
    ...
    plan = build_plan(st, t0, duration, schedule=sched, dt=backend.timestep, suspended=suspended)
    return score(backend.rollout(plan), st)
...
def main(
    ...
    *,
    ...
    backend_cls: Callable[..., Backend],
    load_preset: Callable[[str | None], Preset],
    envelopes: Mapping[str, TorqueEnvelope],
    mjx_backend_cls: Callable[..., Backend] | None = None,
) -> None:
    """Watch (or just score) a Mode-A replay.

        python -m dimos.simulation.sysid.replay REC.mcap --view
    ...
    ap = argparse.ArgumentParser(prog="sysid.replay", description=main.__doc__)
    ...
    ap.add_argument(
        "--engine",
        choices=("mujoco", "mjx") if mjx_backend_cls is not None else ("mujoco",),
        default="mujoco",
    ...
    from dimos.simulation.sysid.recording import read_declarations, read_recording
    from dimos.simulation.sysid.regimes import protected, regimes

    st = read_recording(args.recording, reader)
    ...
    # must never silently run bare (the two are one claim; see presets.Preset).
    envelope = envelopes[preset.envelope] if preset.envelope else None
    if args.engine == "mjx":
        if args.view:
        ...
        assert mjx_backend_cls is not None
        backend: Backend = mjx_backend_cls(envelope=envelope, x64=not args.f32)
    else:
        if args.f32:
