...
class ClassAwareSampler(Sampler):
    def __init__(self, dataset_dicts, seed: Optional[int] = None) -> None:
        """
        Args:
    ...
    def _get_class_balance_factor(self, dataset_dicts, l: float=1.0):
        ...
        for _i, dataset_dict in enumerate(dataset_dicts):
            cat_ids = {ann["category_id"] for ann in dataset_dict["annotations"]}
...
class MultiDatasetSampler(Sampler):
    def __init__(self, cfg, sizes: Sequence[int], dataset_dicts, seed: Optional[int] = None) -> None:
        """
        Args:
            size (int): the total number of data of the underlying dataset to sample from
            seed (int): the initial seed of the shuffle. Must be the same
