...
def cover(points: np.ndarray, cell: float) -> tuple[Grid, bool]:
    ...
    the grid would exceed ``MAX_GRID_CELLS``, the encoder's memory guard; the
    flag reports that it did.
    """
    ...
    while True:
        ...
        if shape[0] * shape[1] <= MAX_GRID_CELLS:
            break
        cell = float(np.ceil(cell * np.sqrt(shape[0] * shape[1] / MAX_GRID_CELLS) * 1000) / 1000)
        limited = True
...
class Overview:
    ...
    XY/Z are cloud axes. Cells are multiples of the cloud's own return spacing:
    ``cell_spacings`` for coverage and structure, ``relief_cell_spacings`` for the
    lower surface, pooled so each cell's percentile has returns behind it. The
    structure band is relative to the median supported lower-return height, not a
    robot pose or a verified floor. Explicit fields remain available for other
    orientations, bands and resolutions.
    """
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("cell_spacings", "relief_cell_spacings"):
    ...
    def run(self, ctx: EncodeContext) -> dict[str, Any]:
        ...
        measured = len(values) >= MIN_SUPPORTED_CELLS and fraction >= MIN_SUPPORTED_FRACTION
