...
class OccupancyMap:
    """The top-down occupancy map of the cloud over the absolute band
    ``z_range``, in this build's form (image or text). The whole cloud unless
    ``zoom`` crops a square; ``mark`` draws a pose on it."""

    z_range: tuple[float, float]  # absolute z: below = free, inside = occupied, above = ignored
    max_cells: int = render.DEFAULT_MAX_CELLS
    zoom: tuple[float, float, float] | None = None  # x, y, radius
    mark: tuple[float, float, float] | None = None  # x, y, yaw_deg
    free_radius: float = render.DEFAULT_FREE_RADIUS_M
    colour: str = render.COLOUR_FLAT
    source: Any = None
    overlays: tuple[Any, ...] = ()
    grid: Grid | None = None

    LEGEND = (
        "OccupancyMap(z_range=(z_low, z_high), max_cells=256, zoom=None | (x, y, radius), "
        "mark=None | (x, y, yaw_deg), free_radius=0, colour='flat' | 'height') -> top-down map of "
        "the whole cloud, or of the square zoom; a return below z_range (absolute z) marks its cell "
        "free, inside it occupied, above it is ignored; free spreads free_radius metres when set. "
        "Cells are spacing_m, the gap between neighbouring returns, or coarser to fit max_cells; "
        "zoom to see a place closely. colour='height' colours each occupied cell by its highest "
        "return over z_range and adds height_scale to read it back. Result: z_range_m, spacing_m, "
        "cell_m, origin_xy (world position of the south-west corner), size [columns, rows], "
        "occupied_cells, free_cells, unseen_cells, mark_cell [column, row] when a mark is given. "
        "An explicit XY grid fixes the origin, resolution, and extent; it cannot be combined "
        "with zoom and must fit max_cells without coarsening."
    )
    ...
    def _fixed_grid(self, ctx: EncodeContext) -> tuple[OccupancyGrid, np.ndarray | None]:
        ...
        cells = np.full(shape, render.UNKNOWN, dtype=np.int8)
        below = points[:, 2] < self.z_range[0]
        occupied = (points[:, 2] >= self.z_range[0]) & (points[:, 2] <= self.z_range[1])
        cells[indices[below, 1], indices[below, 0]] = render.FREE
        cells[indices[occupied, 1], indices[occupied, 0]] = render.OCCUPIED
        free = cells == render.FREE
        if self.free_radius > 0 and free.any():
            distance = ndimage.distance_transform_edt(~free, sampling=spec.cell_m)
            cells[(distance <= self.free_radius) & (cells != render.OCCUPIED)] = render.FREE
        heights = None
        if self.colour == render.COLOUR_HEIGHT:
    ...
    def measure(self, ctx: EncodeContext) -> tuple[OccupancyGrid, np.ndarray | None, np.ndarray]:
        ...
        if self.colour not in render.COLOURS:
            raise ValueError(f"colour must be one of {render.COLOURS}, not {self.colour!r}")
        ...
        if self.grid is None:
            ...
            grid, heights = render.occupancy_grid(
                ...
                zoom=self.zoom,
                heights=self.colour == render.COLOUR_HEIGHT,
    ...
    def run(self, ctx: EncodeContext) -> dict[str, Any]:
        ...
        out: dict[str, Any] = {
            ...
            "colour": self.colour,
            "occupied_cells": int((grid.grid == render.OCCUPIED).sum()),
            "free_cells": int((grid.grid == render.FREE).sum()),
            "unseen_cells": int((grid.grid == render.UNKNOWN).sum()),
        }
        ...
        if render.FORM == "image":
            with ctx.artifact("occupancy.png") as (staging, path):
                ...
                if self.overlays:
                    scale = max(1, round(render.OCCUPANCY_TARGET_PX / max(grid.width, grid.height)))
