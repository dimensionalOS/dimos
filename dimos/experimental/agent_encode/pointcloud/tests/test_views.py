...
def test_depth_image_sees_the_wall_ahead() -> None:
    ...
    depth, _ = views.depth_image(pts, view, size=(64, 40), max_depth=10.0)
    centre = float(depth[18:23, 30:35].min())  # sparse synthetic walls: nearest in a centre block
    assert centre == pytest.approx(3.0, abs=0.15), "the east wall is 3 m ahead"
    # Facing north the wall is 2 m ahead.
    depth_n, _ = views.depth_image(pts, views.View(0.0, 0.0, 1.0, yaw_deg=90.0), size=(64, 40))
    ...
    depth_w, _ = views.depth_image(pts, views.View(0.0, 0.0, 1.0, yaw_deg=180.0), size=(64, 40))
    assert np.isinf(depth_w[0, 32])


def test_depth_ascii_shape_and_digits() -> None:
    pts = room().points_f32()
    depth, _ = views.depth_image(pts, views.View(0.0, 0.0, 1.0), size=(64, 40), max_depth=10.0)
    text = views.depth_ascii(depth, max_depth=None, shape=(32, 8))
    lines = text.split("\n")
...
def test_occupancy_marks_walls_and_places_the_view() -> None:
    ...
    grid, _ = views.occupancy_grid(cloud, pts, spacing=0.25, z_range=(0.1, 2.5), max_cells=128)
    wall_col, wall_row = views.cell_of(grid, 2.9, 0.0)
    assert views.grid_north_up(grid)[wall_row, wall_col] == views.OCCUPIED
    open_col, open_row = views.cell_of(grid, -2.0, 0.0)
    assert views.grid_north_up(grid)[open_row, open_col] != views.OCCUPIED
...
def test_max_cells_coarsens_the_grid() -> None:
    ...
    grid, _ = views.occupancy_grid(
        cloud, cloud.points_f32(), spacing=0.05, z_range=(0.1, 2.5), max_cells=20
    )
