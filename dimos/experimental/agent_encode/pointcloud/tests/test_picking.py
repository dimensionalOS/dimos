...
@pytest.fixture
def view():
    return pc.DepthView((0, 0, 1, 0, 0), size=(100, 100), point_size_m=0.4)
...
def test_depth_returns_original_point_through_splat_and_fill(cloud, view, uv, provenance, tmp_path):
    ...
    picked = result(cloud, pc.Pick(ref, uv=uv), tmp_path)

    assert picked["outcome"] == "hit"
    assert picked["hits"][0]["point_m"] == [2, 0, 0]
    assert picked["hits"][0]["forward_depth_m"] == 2
    assert picked["hits"][0]["projected_uv"] == [50, 75]
    assert picked["hits"][0]["pixel_provenance"] == [provenance]
...
def test_projection_uses_camera_pitch_heading_and_cloud_frame(yaw, pitch, tmp_path):
    ...
    view = pc.DepthView((1, -3, 2, yaw, pitch), size=(100, 100), point_size_m=0)
    ref = result(cloud, view, tmp_path)["view_ref"]

    picked = result(cloud, pc.Pick(ref, uv=(53, 48)), tmp_path)

    assert picked["hits"][0]["point_m"] == pytest.approx(point)
    assert picked["hits"][0]["forward_depth_m"] == pytest.approx(3)
    assert picked["hits"][0]["projected_uv"] == pytest.approx([53 + 1 / 3, 48 + 1 / 3])


def test_occlusion_ties_missing_and_clipped_neighborhood(cloud, view, tmp_path):
    ref = result(cloud, view, tmp_path)["view_ref"]
    picked = result(cloud, pc.Pick(ref, uv=(50, 75), radius_px=5), tmp_path)
    assert picked["selected_return_count"] == 1
    assert picked["hits"][0]["point_id"] == 0
    assert picked["depth_span_m"] == [2, 2]
    assert result(cloud, pc.Pick(ref, uv=(0, 0), radius_px=2), tmp_path)["selected_pixels"] == 9
    assert result(cloud, pc.Pick(ref, uv=(0, 0)), tmp_path)["outcome"] == "no_return"
    assert result(cloud, pc.Pick(ref, uv=(100, 0)), tmp_path)["outcome"] == "outside_image"
    ...
    assert result(duplicate, pc.Pick(ref, uv=(50, 75)), tmp_path)["hits"][0]["point_id"] == 0
...
def test_distinct_region_hits_report_span_and_keep_all_returns(tmp_path):
    ...
    view = pc.DepthView((0, 0, 0, 0, 0), size=(100, 100), point_size_m=0)
    ref = result(cloud, view, tmp_path)["view_ref"]
    picked = result(cloud, pc.Pick(ref, rect=(40, 45, 30, 10), max_items=1), tmp_path)
    assert picked["outcome"] == "ambiguous"
    assert picked["selected_return_count"] == 2 and picked["hits_omitted"] == 1
    assert picked["depth_span_m"] == [2, 4]
    selection = pc.SelectionRef(picked["selection_ref"])
    assert (
        result(cloud, pc.Overlap(pc.Sphere((0, 0, 0), 10), source=selection), tmp_path)["count"]
        == 2
    )
...
def test_height_cell_bounds_north_up_edges_and_selected_source(plane, tmp_path):
    ...
    cloud = PointCloud2.from_numpy(points, frame_id="custom")
    grid = pc.Grid((-1, -1), (2, 2), 0.5, plane=plane, frame="custom")
    source = pc.Select(include=pc.Band("xyz"[normal], None, 8))
    field = pc.HeightField(grid, source)
    ref = result(cloud, pc.Map(field.min, max_side=20), tmp_path)["view_ref"]
    picked = result(cloud, pc.Pick(ref, uv=(0, 10)), tmp_path)
    cell = picked["cells"][0]
    ...
    edge = result(cloud, pc.Pick(ref, uv=(10, 10)), tmp_path)["cells"][0]
    assert edge["cell"] == [1, 0] and edge["count"] == 1 and edge["value"] == 6
    missing = result(cloud, pc.Pick(ref, uv=(0, 0)), tmp_path)
    assert missing["outcome"] == "no_return"
...
def test_derived_field_values_do_not_claim_remote_targets_as_local_points(tmp_path):
    cloud = PointCloud2.from_numpy(np.array([[5, 5, 1]], dtype=np.float32))
    grid = pc.Grid((0, 0), (2, 2), 1)
    distance = pc.DistanceField(grid)
    for field, expected in (
        (distance, np.hypot(4.5, 4.5)),
        (pc.Threshold(distance, ">", 1), 1),
        (pc.Components(pc.Threshold(distance, ">", 1)), 1),
    ):
        ref = result(cloud, pc.Map(field, max_side=20), tmp_path)["view_ref"]
        picked = result(cloud, pc.Pick(ref, uv=(0, 10)), tmp_path)
        assert picked["outcome"] == "field_value"
...
def test_occupancy_fixed_grid_band_height_and_dilated_empty_cells(tmp_path):
    ...
    grid = pc.Grid((0, 0), (3, 2), 1)
    view = pc.OccupancyMap((1, 3), grid=grid, free_radius=1, colour="height")
    ref = result(cloud, view, tmp_path)["view_ref"]
    scale = round(1024 / 3)
    occupied = result(cloud, pc.Pick(ref, uv=(scale, scale)), tmp_path)["cells"][0]
    assert occupied["value"] == "occupied"
    assert occupied["count"] == 1 and occupied["max_m"] == 2
    empty = result(cloud, pc.Pick(ref, uv=(0, 0)), tmp_path)
    assert empty["outcome"] == "no_return"
    assert empty["cells"][0]["value"] == "free"
...
def test_occupancy_crop_uses_actual_origin_and_builder_cell_ownership(tmp_path):
    ...
    view = pc.OccupancyMap((0.5, 3), zoom=(0.6, 0.4, 0.4), max_cells=32, colour="height")
    output = result(cloud, view, tmp_path)
    picked = result(cloud, pc.Pick(output["view_ref"], uv=(0, 0)), tmp_path)
    spec = picked["grid"]
...
def test_polygon_and_rectangle_deduplicate_and_limits_are_contained(cloud, view, tmp_path):
    ref = result(cloud, view, tmp_path)["view_ref"]
    rectangle = result(cloud, pc.Pick(ref, rect=(45, 70, 11, 11)), tmp_path)
    polygon = result(
        cloud, pc.Pick(ref, polygon=((45, 70), (56, 70), (56, 81), (45, 81))), tmp_path
    )
    assert rectangle["selected_return_count"] == polygon["selected_return_count"] == 1
    assert rectangle["selected_pixels"] == polygon["selected_pixels"] == 121
    for bad in (
        pc.Pick(ref, uv=(1, 1), rect=(0, 0, 2, 2)),
        pc.Pick(ref, uv=(1.5, 2)),
        pc.Pick(ref, uv=(1, 1), radius_px=65),
        pc.Pick(ref, rect=(-1, 0, 3, 3)),
        pc.Pick(ref, polygon=((0, 0), (1, 1))),
        pc.Pick(ref, uv=(1, 1), max_items=65),
    ):
        assert result(cloud, bad, tmp_path)["status"] == "invalid"
    large = result(cloud, replace(view, size=(300, 300)), tmp_path)["view_ref"]
    assert result(cloud, pc.Pick(large, rect=(0, 0, 300, 300)), tmp_path)["status"] == "invalid"


def test_json_roundtrip_in_new_process_and_nested_selected_view(cloud, view, tmp_path):
    ref = result(cloud, view, tmp_path)["view_ref"]
    pick = pc.Pick(ref, uv=(50, 75))
    selected = result(cloud, pick, tmp_path)["selection_ref"]
    code = """import json,sys,numpy as np
from dimos.experimental.agent_encode.pointcloud import api as pc
from dimos.msgs.sensor_msgs.PointCloud2 import PointCloud2
p=json.loads(sys.stdin.read())
c=PointCloud2.from_numpy(np.array([[2,0,0],[4,0,-1]],dtype=np.float32),frame_id='sensor',timestamp=7)
s=pc.SelectionRef(p['selection'])
v=pc.DepthView((0,0,1,0,0),size=(100,100),point_size_m=.4,source=s)
r=c.agent_encode({'v':v},out_dir=p['out'])['results']['v']
print(json.dumps(c.agent_encode({'p':pc.Pick(r['view_ref'],uv=(50,75))},out_dir=p['out'])))
"""
    ...
    assert picked["status"] == "ok" and picked["hits"][0]["point_m"] == [2, 0, 0]
...
def test_selection_composes_with_every_point_source_handler(cloud, view, tmp_path):
    ref = result(cloud, view, tmp_path)["view_ref"]
    pick = pc.Pick(ref, uv=(50, 75))
    source = pc.Select(source=pick.selection, include=pc.Band("z", -0.1, 0.1))
    grid = pc.Grid((1, -1), (2, 2), 1)
    sphere = pc.Sphere((2, 0, 0), 0.1)
    requests = {
        "closest": pc.Closest(sphere, source=source),
        "overlap": pc.Overlap(sphere, source=source),
        "sweep": pc.Sweep(sphere, direction_deg=0, max_distance=1, source=source),
        "depth": replace(view, source=source),
        "occupancy": pc.OccupancyMap((-0.1, 0.1), source=source, grid=grid),
        "height": pc.Sample(pc.HeightField(grid, source), [(2, 0)]),
        "distance": pc.Sample(pc.DistanceField(grid, source), [(2, 0)]),
    }
    output = cloud.agent_encode(requests, out_dir=tmp_path, budget=pc.EncodeBudget(65536))[
        "results"
    ]
...
def test_overlay_confirmation_and_budget_preserve_measurement(cloud, view, tmp_path):
    ...
    pick = pc.Pick(plain["view_ref"], uv=(50, 80))
    ...
    small = cloud.agent_encode({"pick": pick}, out_dir=tmp_path, budget=pc.EncodeBudget(1024))
    assert small["results"]["pick"]["status"] == "too_large"
    assert len(json.dumps(small).encode()) <= 1024
    after = result(cloud, pick, tmp_path)
    assert after == before
    grid = pc.Grid((1, -1), (2, 2), 1)
    map_view = pc.Map(pc.HeightField(grid).min, max_side=20)
    map_ref = result(cloud, map_view, tmp_path)["view_ref"]
    map_pick = pc.Pick(map_ref, uv=(10, 0))
    assert (
        "picked_pixels"
...
def test_reference_validation_refuses_tampering_and_nested_overflow(cloud, view, tmp_path):
    ref = result(cloud, view, tmp_path)["view_ref"]
    tampered = json.loads(json.dumps(ref))
    tampered["recipe"]["size"] = [10, 10]
    assert result(cloud, pc.Pick(tampered, uv=(0, 0)), tmp_path)["status"] == "invalid"
    nested = {}
    nested["loop"] = nested
    assert result(cloud, pc.Pick(nested, uv=(0, 0)), tmp_path)["status"] == "invalid"


def test_three_visual_pick_cycles_keep_original_cloud_binding(cloud, view, tmp_path):
    grid = pc.Grid((1, -1), (2, 2), 1)
    ref = result(cloud, pc.Map(pc.HeightField(grid).max, max_side=20), tmp_path)["view_ref"]
    picked = result(cloud, pc.Pick(ref, uv=(10, 0)), tmp_path)
    for _ in range(3):
        source = pc.SelectionRef(json.loads(json.dumps(picked["selection_ref"])))
        filtered_view = result(cloud, replace(view, source=source), tmp_path)
        assert filtered_view["status"] == "ok"
        picked = result(cloud, pc.Pick(filtered_view["view_ref"], uv=(50, 75)), tmp_path)
        assert picked["status"] == "ok" and picked["hits"][0]["point_m"] == [2, 0, 0]
