...
class PythonNativeModuleConfig(NativeModuleConfig):
    """Process settings for an isolated Python-native module."""

    # Python-native modules resolve their real command from the sibling project.
    executable: str = "uv"
    startup_timeout: float = 30.0
    output_limit: int = 64 * 1024


def contract_rpc_names(module_class: type[PythonNativeModule]) -> frozenset[str]:
    """Return RPC names introduced below PythonNativeModule in the MRO."""

    framework = set(PythonNativeModule.rpcs)
    return frozenset(set(module_class.rpcs) - framework)


class PythonNativeModule(NativeModule):
    ...
    _python_native_runtime: bool
    _runtime_client: RPCClient | None
    _runtime_name: str | None
    _module_refs: dict[str, RPCClient]

    def __init__(self, _python_native_runtime: bool = False, **kwargs: Any) -> None:
        self._python_native_runtime = _python_native_runtime
    ...
    def runtime_project(self) -> Path:
        ...
        if not (project / "pyproject.toml").is_file():
            raise FileNotFoundError(
                f"Python-native runtime manifest is missing: {project / 'pyproject.toml'}"
            )
        return project
    ...
    def _launch_command(self, handshake_fd: int) -> list[str]:
        ...
        args.extend(
            [
                "python",
                "-m",
                "dimos.core.python_native_bootstrap",
                "--declaration",
    ...
    def _new_runtime_name(self) -> str:
        ...
        self._runtime_name = f"__python_native__/{public_name}/{short_id()}"
    ...
    def _run_prepare(self) -> None:
        ...
        if result.returncode:
            ...
            raise RuntimeError(
                f"Python-native environment preparation failed (exit {result.returncode}): "
    ...
    def _spawn_runtime(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Starting Python-native runtime",
            ...
            watchdog = threading.Thread(
                ...
                daemon=True,
                name=f"python-native-watchdog-{type(self).__name__}",
            )
            ...
            with os.fdopen(parent_read, "rb") as ready:
                ...
                while time.monotonic() < deadline:
                    if select.select([ready], [], [], 0.1)[0]:
                        ...
                        if message == "READY":
                            return
                        raise RuntimeError(f"Python-native runtime failed to start: {message}")
                    if self._process.poll() is not None:
                        break
            raise RuntimeError("Python-native runtime exited before becoming ready")
    ...
    def _connect_runtime(self) -> None:
        if self._runtime_name is None:
            raise RuntimeError("Python-native runtime did not choose an RPC name")
    ...
    @rpc
    def build(self) -> None:
        if self._python_native_runtime:
            Module.build(self)
            return
        Module.build(self)
        self._run_prepare()
    ...
    def set_transport(self, stream_name: str, transport: Any) -> bool:
        ...
        if not self._python_native_runtime and self._runtime_client is not None:
    ...
    def stop(self) -> None:
        ...
        if client is not None:
            try:
                ...
                logger.exception("Failed to stop Python-native runtime module")
            finally:
