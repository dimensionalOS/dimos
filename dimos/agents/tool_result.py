...
"""Structured return type for ``@tool`` methods.

Tools historically returned free-form strings, which forced agents to parse
prose to tell success from failure. ``ToolResult`` carries a typed
...
declares its own alias (see e.g. ``dimos.manipulation.tool_errors``) and a
tool annotates ``ToolResult[DomainError]`` so the type checker enforces that
only that domain's codes are emitted. ``CommonToolError`` holds codes that
...
class ToolResult(Generic[E]):
    ...
    @classmethod
    def ok(cls, message: str = "", **metadata: Any) -> ToolResult[E]:
        return cls(success=True, message=message, metadata=dict(metadata))

    @classmethod
    def fail(cls, error_code: E, message: str = "") -> ToolResult[E]:
