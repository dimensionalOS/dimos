...
class ClaudeAgent(LLMAgent):
    ...
    def _build_prompt(
        ...
        if rag_results:
            messages.append(
                {"role": "user", "content": f"{rag_results}\n\n{self.query}"}
            )
            logger.info(
    ...
    def _send_query(self, messages: list, claude_params: dict) -> Any:
        ...
        try:
            ...
            claude_params = (
                claude_params.get("claude_prompt", None) or self.claude_api_params
            )
            ...
            with self.client.messages.stream(**claude_params) as stream:
                ...
                with open(
                    os.path.join(self.output_dir, "memory.txt"), "a"
                ) as memory_file:
        ...
        try:
            ...
            if response_message.content:
                content_blocks.append(
                    {"type": "text", "text": response_message.content}
                )
            if content_blocks:
