...
def test_physical_insufficiency_flag_set_when_pins_too_big() -> None:
    ...
    out = assemble_prompt([sys, e1, e2], budget=_budget(500))
...
def test_phase2_upgrades_recent_pages_first() -> None:
    ...
    out = assemble_prompt([p_old, p_mid, p_new], budget=_budget(50))
...
def test_phase2_never_exceeds_budget() -> None:
    ...
    for i in range(20):
        tokens = tuple(
            sorted(
                [rng.randint(1, 5), rng.randint(6, 20), rng.randint(21, 80), rng.randint(100, 1200)]
            )
        )
...
def test_tool_call_pair_never_broken_by_eviction() -> None:
    ...
    out = assemble_prompt([other, ai, tool], budget=_budget(100, output_reserve=97, overhead=0))
...
def test_tool_call_pair_degrades_together_when_possible() -> None:
    ...
    out = assemble_prompt([ai, tool], budget=_budget(100, output_reserve=97, overhead=0))
...
def test_messages_preserve_ingest_order() -> None:
    ...
    assert next(isinstance(m, SystemMessage) for m in out.messages) is True
...
def test_assemble_accounts_per_message_overhead() -> None:
    ...
    """
    pages = [_conv_page(f"p{i}", i, tokens=(100, 100, 100, 100)) for i in range(10)]
    budget = _budget(1000)
...
def test_assemble_recomputes_cap_after_eviction() -> None:
    ...
    """
    pinned_pages = [
        _evidence_page(f"pin{i}", i, pinned=True, tokens=(107, 107, 107, 107)) for i in range(9)
    ]
...
def test_random_mixes_never_exceed_budget_when_pins_are_small() -> None:
    ...
    for trial in range(50):
        ...
        out: AssembledPrompt = assemble_prompt(
            pages, budget=_budget(cap + 0, output_reserve=0, overhead=0)
        )
        if out.physical_insufficient:
            continue
        assert out.total_tokens <= cap, (
            f"trial {trial}: total={out.total_tokens} cap={cap} chosen={out.chosen_levels}"
        )
