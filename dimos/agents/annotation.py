...
def current_tool_context() -> dict[str, Any] | None:
    """Return the per-call context for the currently executing `@tool`.

    Returns a (possibly empty) dict inside a `@tool` call and `None` when no
    tool is currently on the stack in this thread. The MCP server populates
    `{"progress_token": <token>}` when the caller supplied
    `params._meta.progressToken`. Otherwise the dict is empty. Downstream code
    uses the `None` vs. `{}` distinction to tell "outside any tool" from
    "inside a tool that didn't get a token."
    """
    return getattr(_TOOL_CONTEXT, "context", None)


def _stamp_and_log(func_name: str, result: Any, elapsed_ms: float) -> Any:
    """If ``result`` is a ``ToolResult``, attach the elapsed duration and log.

    Returns the (possibly new) result. Tools returning non-ToolResult values
    ...
    if isinstance(result, ToolResult):
        ...
        # Not a ToolResult — we can't verify the outcome, so don't claim "OK".
        code = "UNKNOWN"
    logger.info("TOOL %s result=%s duration_ms=%.1f", func_name, code, elapsed_ms)
    return result
...
def tool(func: F) -> F:
    if inspect.iscoroutinefunction(func):
        ...
        async def async_context_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            previous = getattr(_TOOL_CONTEXT, "context", None)
            ...
            try:
                ...
                logger.info("TOOL %s result=EXCEPTION duration_ms=%.1f", func.__name__, elapsed_ms)
                raise
            finally:
                _TOOL_CONTEXT.context = previous
    ...
    wrapped.__tool__ = True  # type: ignore[attr-defined]
