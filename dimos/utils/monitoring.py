...
class GilMonitorThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            print(
                f"Error: 'py-spy' command not found. Please ensure py-spy is installed and in your PATH.",
                flush=True,
            )
        except Exception as e:
            print(f"An error occurred in GILMonitorThread for PID {self.pid}: {e}", flush=True)
        finally:
            if process:
                process.terminate()
