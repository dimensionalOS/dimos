...
"""Ivan's pure-module PGO, adapted to the jnav loop-closure eval harness.

Ivan's online PGO (feat/ivan/pure-modules, `dimos/pure/modules/pgo_mapper.py`) is a
thin wrapper around the incremental ISAM2+ICP core `_PGOState` in
`dimos/mapping/loop_closure/pgo.py`. That core is the algorithm Jeff wants
benchmarked against the Rust `gsc_pgo`. This file is the same online-PGO logic as
Ivan's `PGOVoxelMapper.step`, but instead of building a voxel map it emits exactly
what eval.py's replay harness (`run_module_graph` / `GraphCapture`) consumes:
`corrected_odometry` (per-scan ack + drift correction), `pose_graph` (Graph3D of the
optimized keyframes), and `loop_closure_event` (one GraphDelta3D per accepted loop).
...
knobs don't apply; Ivan's PGO runs on its own PGOConfig (Go2-tuned defaults).
"""
...
class IvanPGOConfig(ModuleConfig):
    world_frame: str = "map"
...
class IvanPGO(Module):
    """Online ISAM2+ICP PGO (Ivan's `_PGOState`) wired to the eval harness streams."""

    config: IvanPGOConfig
