...
# Minimum corridor check length (walk-speed baseline).
PATH_CLEARANCE_LOOKUP_DISTANCE_M = 3.0
# Scale lookahead so faster profiles keep ~constant reaction time.
PATH_CLEARANCE_LOOKUP_TIME_S = 5.0
# Treat elevated costmap cells as blocking (matches navigation gradient threshold).
PATH_CLEARANCE_OBSTACLE_COST_THRESHOLD = 50


def path_clearance_lookup_distance_m(speed_m_s: float) -> float:
    """Return speed-scaled path-corridor check distance in meters."""
    if not math.isfinite(speed_m_s) or speed_m_s <= 0.0:
        return PATH_CLEARANCE_LOOKUP_DISTANCE_M
    return max(PATH_CLEARANCE_LOOKUP_DISTANCE_M, PATH_CLEARANCE_LOOKUP_TIME_S * speed_m_s)


class PathClearance:
    _costmap: OccupancyGrid | None = None
    _last_costmap: OccupancyGrid | None = None
    _path_lookup_distance: float
    _obstacle_cost_threshold: int = PATH_CLEARANCE_OBSTACLE_COST_THRESHOLD
    ...
    def __init__(self, global_config: GlobalConfig, path: Path) -> None:
        ...
        self._pose_index = 0
        self._lock = RLock()
        self._path_lookup_distance = PATH_CLEARANCE_LOOKUP_DISTANCE_M

    def set_planner_speed(self, speed_m_s: float) -> None:
        lookup_distance = path_clearance_lookup_distance_m(speed_m_s)
        with self._lock:
            if abs(lookup_distance - self._path_lookup_distance) > 0.01:
                self._path_lookup_distance = lookup_distance
    ...
    def is_obstacle_ahead(self) -> bool:
        ...
        return bool(
            np.any(
                (cells >= self._obstacle_cost_threshold) & (cells != CostValues.UNKNOWN)
            )
        )
