...
from dimos.navigation.dannav.local_planner import LocalPlanner, StopMessage
...
class GlobalPlanner(Resource):
    ...
    _goal_run_profile: str | None = None
    ...
    _replan_on_costmap_update: bool
    # Target ~0.14 m between hot replans; interval = spacing / speed (clamped).
    _inplace_replan_spacing_m: float = 0.14
    _inplace_replan_min_interval_floor_s: float = 0.05
    _inplace_replan_min_interval_ceil_s: float = 0.25
    _last_inplace_replan_monotonic: float = 0.0

    def __init__(self, global_config: GlobalConfig, *, replan_on_costmap_update: bool = False) -> None:
        self.path = Subject()
        self.goal_reached = Subject()

        self._global_config = global_config
        self._replan_on_costmap_update = replan_on_costmap_update
        ...
        self._local_planner = LocalPlanner(
            ...
            hot_replan=replan_on_costmap_update,
        )
    ...
    def handle_global_costmap(self, msg: OccupancyGrid) -> None:
        ...
        if not self._replan_on_costmap_update:
            return
        ...
        with self._lock:
            self._replan_reason = "map_updated"
        self._replan_event.set()

    def handle_goal_request(self, goal: PoseStamped, run_profile: str | None = None) -> None:
        """Plan toward ``goal``; ``run_profile`` overrides the session envelope for it.

        The per-goal profile sticks to the goal across replans and is cleared
        when the goal is cancelled or reached, so the next goal falls back to
        the session default.
        """
        logger.info("Got new goal", goal=str(goal), run_profile=run_profile)
        with self._lock:
            self._current_goal = goal
            self._goal_run_profile = run_profile
            self._goal_reached = False
        self._replan_limiter.reset()
        self._plan_path()

    def set_run_profile(self, profile: str) -> None:
        """Set the session-default run profile used for subsequent goals.

        Module configs are per-process copies, so the operator surface pushes
        the chosen profile here instead of relying on its own GlobalConfig
        write being visible to the planner.
        """
        self._global_config.go2_run_profile = profile
    ...
    def _handle_stop_message(self, stop_message: StopMessage) -> None:
        ...
        if stop_message in ("map_updated", "obstacle_found") and self._replan_on_costmap_update:
            if stop_message == "obstacle_found":
                logger.info("Hot replanning path due to obstacle found.")
            else:
                logger.info("Hot replanning path due to costmap update.")
            self._replan_path_inplace()
            return
        ...
        if stop_message == "obstacle_found":
            ...
            self._replan_path()
        elif stop_message == "run_envelope_rejected":
            # Unknown run profile or other envelope resolution failure; replanning
            # would not change the outcome, so cancel the goal.
            logger.warning("Run profile could not be applied; cancelling goal.")
            self.cancel_goal()
        else:
    ...
    def _inplace_replan_min_interval_for_speed(self, speed_m_s: float) -> float:
        """Shorter throttle at higher planner speed (~fixed spatial replan spacing)."""
        if not math.isfinite(speed_m_s) or speed_m_s <= 0.0:
            return self._inplace_replan_min_interval_ceil_s
        interval = self._inplace_replan_spacing_m / speed_m_s
        return max(
            self._inplace_replan_min_interval_floor_s,
            min(self._inplace_replan_min_interval_ceil_s, interval),
        )

    def _replan_path_inplace(self) -> None:
        now = time.monotonic()
        min_interval = self._inplace_replan_min_interval_for_speed(
            self._local_planner.planner_speed_m_s()
        )
        if now - self._last_inplace_replan_monotonic < min_interval:
            return
        self._last_inplace_replan_monotonic = now

        with self._lock:
            current_odom = self._current_odom
            current_goal = self._current_goal
            goal_run_profile = self._goal_run_profile
        ...
        if not safe_goal:
            logger.warning(
                "No safe goal found during hot replan.",
        ...
        if not path:
            logger.warning(
                "No path found during hot replan.",
        ...
        if self._local_planner.get_state() == NavigationState.IDLE:
            self._local_planner.start_planning(resampled_path, run_profile_name=goal_run_profile)
            return

        if not self._local_planner.update_path(resampled_path):
            self._local_planner.start_planning(resampled_path, run_profile_name=goal_run_profile)
