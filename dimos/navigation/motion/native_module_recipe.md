...
`dimos/core/native_module.py`, `dimos/core/baked_host.py`. `dimos bake` landed on
main as **#3490** (`ffcc88cdc`), which is the version this recipe was re-checked
against; the prototype it grew out of (`5f7de46a5` module-lib split, port registry,
host runtime; `b619a7b6f` the command and `baked_host()`) is only on
`ivan/feat/trajectory_ctrl`.
...
- **Q1 — one crate or two for planner+follower: ONE crate,
  ...
  in one manifest (`discovery.py:parse_manifest`), and the macro cross-check is per-id too,
...
- **Q3 — genuinely open items are marked OPEN inline.** The big one: how tuned
  ...
  *python class defaults* (`cli.py:default_config` instantiates
...
Keys, per `discovery.py:parse_manifest`:

| key | required | meaning |
|---|---|---|
| `path` | yes (string) | full rust path of the struct, used verbatim in the generated host's `ModuleEntry::new::<{path}>(...)` (`codegen.py:_entries`) |
| `python` | yes (string) | `pkg.mod:Class` of the python `NativeModule` wrapper; imported by `--emit-config` (`cli.py:default_config`). This is what makes every bake module a standalone module too — the key is required, so a wrapper class must exist. |
...
only (`discovery.py:normalize_id`). An id declared twice anywhere under `dimos/`
or `native/` is a `BakeError` (`discovery.py:discover_modules`).
...
`codegen.py:_dependencies` generates the host's dependency
...
Built python-side by `transport_topic` (`"dimos/" + name`) plus
`ZenohTopic.key_expr` appending `lcm_type.msg_name`. Bake does NOT keep its own
copy of that rule any more: `graph.py` imports `zenoh_key_expr` from
`core/transport_factory.py` and builds the key with it, so a baked host cannot
drift onto keys nobody else uses -- the two sides are the same function.
...
- **Same channel name.** Autoconnect wires ports by name; blueprints remap
  ...
  `topics` map (`dimos-module/src/module.rs`, `Builder::topic_for`).
...
QoS: same story — `graph.py` calls `default_zenoh_qos_for` from
`core/transport_factory.py`, so PointCloud2/Image channels get latest-wins on
both sides. Twist/Bool/Path channels ride the defaults; nothing to do.
...
1. **The stale `result/bin` nix default.** `RayTracingVoxelMapConfig` builds
   ...
   discovery — `discovery.py:_PRUNE` — so manifests inside them never register.)
2. **`--emit-config` emits python class *defaults*, not blueprint values.**
   `default_config` instantiates `config_type()` (`cli.py`). Anything
...
12. **One instance per host.** `select_modules` refuses duplicate ids
    (`discovery.py:select_modules`). Known limit; not blocking for this stack.
