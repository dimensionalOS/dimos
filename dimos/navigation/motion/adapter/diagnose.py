...
def plans(rec: Recording) -> list[dict[str, float]]:
    ...
    # the field twin of the battery's `rerolls` column.
...
# is a re-roll: the map did not change, so the planner changed its mind, not its
# information. The referee's judge prices the same edge at the same number.
REROLL_M = 0.15
...
def write_plots(
    ...
    """Churn / flip / age / twist time series as SVG (dimos.memory.vis.plot)."""
