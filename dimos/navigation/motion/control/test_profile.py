...
def test_governor_curve_endpoints() -> None:
    v = governor_speed(np.array([FLOOR_CLEARANCE, SPEED_CLEARANCE, 99.0, 0.0]))
...
def test_roundtrip_recovers_governor_speeds() -> None:
    ...
    encode_precision(path, clearance, t0=5.0)
    ceilings = decode_ceilings(path)
    assert ceilings is not None
    v = governor_speed(clearance)
...
def test_tight_zone_reads_slow() -> None:
    ...
    encode_precision(path, clearance)
    ceilings = decode_ceilings(path)
    assert ceilings is not None
...
def test_unstamped_path_decodes_none() -> None:
    assert decode_ceilings(_path()) is None  # flat ts everywhere


def test_non_monotone_decodes_none() -> None:
    path = _path()
    encode_precision(path, np.full(len(path.poses), 1.0))
    path.poses[5].ts = 999.0
    assert decode_ceilings(path) is None
...
def test_fan_keeps_timeline_monotone_and_inherits_ceiling() -> None:
    ...
    encode_precision(path, np.full(len(poses), 1.0))
...
def test_empty_and_single_pose() -> None:
    ...
    assert decode_ceilings(encode_precision(empty, np.zeros(0))) is None
    single = Path(frame_id="odom", poses=[_pose(0, 0)])
    assert decode_ceilings(encode_precision(single, np.zeros(1))) is None
...
def test_stall_never_reads_as_schedule() -> None:
    ...
    encode_precision(a, clearance, t0=0.0)
    encode_precision(b, clearance, t0=12345.6)
    ca, cb = decode_ceilings(a), decode_ceilings(b)
