...
research generation lands by replacing its own track's module and flipping the
one line in `TRACKS` that names it.

Each `*-rs` law is a port of its python twin, not a variant: `test_rust_parity.py`
holds every pair to 1e-9 per twist component.
