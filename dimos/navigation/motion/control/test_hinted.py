...
that has to agree with a constant the referee owns: rust cannot import
``simulation/walk.py``, so a plant number copied into the law is a number that
can silently drift away from the plant.
"""
...
from dimos.navigation.motion.simulation.walk import COMMAND_SLEW, CONTROL_DT
...
def test_rate_limit_is_the_plants_own_slew() -> None:
    ...
    ``COMMAND_SLEW`` is per control tick; the law carries a RATE so it stays
    correct at any control period. If the sim's slew or rate ever moves, this
    fails rather than leaving the law quietly modelling the old plant.
    """
    want = tuple(np.asarray(COMMAND_SLEW, dtype=float) / CONTROL_DT)
