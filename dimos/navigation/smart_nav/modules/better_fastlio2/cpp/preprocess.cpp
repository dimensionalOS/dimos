...
/**
 * @brief Process a Velodyne point cloud
 */
void Preprocess::process(const pcl::PointCloud<velodyne_ros::Point>& pl_orig, pcl::PointCloud<PointType>::Ptr& pcl_out)
{
  switch (time_unit)
  {
    case SEC:
  ...
  velodyne_handler(pl_orig);
  *pcl_out = pl_surf;
}

/**
 * @brief Process an Ouster point cloud
 */
void Preprocess::process(const pcl::PointCloud<ouster_ros::Point>& pl_orig, pcl::PointCloud<PointType>::Ptr& pcl_out)
{
  switch (time_unit)
  {
    case SEC:
  ...
  oust64_handler(pl_orig);
  *pcl_out = pl_surf;
}

/**
 * @brief Process a Livox (ROS-format) point cloud
 */
void Preprocess::process(const pcl::PointCloud<livox_ros::Point>& pl_orig, pcl::PointCloud<PointType>::Ptr& pcl_out)
{
  livoxros_handler(pl_orig);
  *pcl_out = pl_surf;
}
...
void Preprocess::livoxros_handler(const pcl::PointCloud<livox_ros::Point>& pl_orig)
{
  /* Clear previous point cloud cache */
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
