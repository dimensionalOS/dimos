...
"""Tests for PGO (Pose Graph Optimization) module.

Tests the Python reference implementation of the PGO algorithm, covering:
- Keyframe detection
...
try:
    ...
    from dimos.navigation.smart_nav.modules.pgo.pgo_reference import PGOConfig, SimplePGOReference

    _HAS_PGO_DEPS = True
except ImportError:
    _HAS_PGO_DEPS = False

pytestmark = pytest.mark.skipif(not _HAS_PGO_DEPS, reason="gtsam/open3d not installed")
...
class TestKeyframeDetection:
    ...
    def test_first_pose_is_always_keyframe(self):
        ...
        assert len(pgo.key_poses) == 1

    def test_small_movement_not_keyframe(self):
        """A pose very close to the last keyframe should be rejected."""
        pgo = SimplePGOReference(PGOConfig(key_pose_delta_trans=0.5, key_pose_delta_deg=10.0))
        cloud = make_random_cloud(np.zeros(3), seed=0)
...
class TestLoopClosure:
    ...
    def _build_square_trajectory(
        ...
        at each pose. Calls search_for_loop_pairs() on each keyframe.
        """
        t = 0.0
        positions = []
    ...
    def test_loop_closure_detected_on_revisit(self):
        ...
        assert len(pgo.history_pairs) > 0, (
            f"No loop closure detected with {len(pgo.key_poses)} keyframes. "
            f"Start pos: {pgo.key_poses[0].t_global}, "
            f"End pos: {pgo.key_poses[-1].t_global}"
        )
    ...
    def test_no_false_loop_closure(self):
        ...
        assert len(pgo.history_pairs) == 0, "False loop closure on straight line"
    ...
    def test_loop_closure_respects_time_threshold(self):
        ...
        assert len(pgo.history_pairs) == 0, "Loop closure triggered despite time threshold not met"
    ...
    def test_loop_closure_corrects_drift(self):
        ...
        assert len(pgo.key_poses) > 0
        assert len(pgo.key_poses) >= 10
...
class TestGlobalMap:
    ...
    def test_global_map_updates_after_loop_closure(self):
        ...
        if len(pgo.key_poses) >= 2:
            from dimos.navigation.smart_nav.modules.pgo.pgo_reference import LoopPair

            pgo._cache_pairs.append(
                LoopPair(
                    source_id=len(pgo.key_poses) - 1,
                    target_id=0,
                    r_offset=np.eye(3),
                    t_offset=np.zeros(3),
                    score=0.1,
    ...
    def test_global_map_is_published_as_pointcloud(self):
        ...
        pgo = SimplePGOReference(PGOConfig(key_pose_delta_trans=0.3))
...
class TestICP:
    ...
    def test_icp_matches_identical_clouds(self):
        ...
        transform, score = pgo._run_icp(cloud, cloud)
    ...
    def test_icp_matches_translated_cloud(self):
        ...
        pgo = SimplePGOReference(PGOConfig(max_icp_correspondence_dist=5.0))
        cloud = make_structured_cloud(np.zeros(3), n_points=500, seed=42)
        shifted = cloud + np.array([1.0, 0.0, 0.0])

        transform, score = pgo._run_icp(shifted, cloud)
        # The transform should move the shifted cloud back toward the original
    ...
    def test_icp_rejects_dissimilar_clouds(self):
        """ICP between very different clouds should fail to match."""
        SimplePGOReference(PGOConfig(max_icp_correspondence_dist=2.0))

        # Two clouds in completely different locations
        ...
        result = o3d.pipelines.registration.registration_icp(
            o3d.geometry.PointCloud(o3d.utility.Vector3dVector(cloud_a)),
            o3d.geometry.PointCloud(o3d.utility.Vector3dVector(cloud_b)),
            max_correspondence_distance=2.0,
            init=np.eye(4),
            estimation_method=o3d.pipelines.registration.TransformationEstimationPointToPoint(),
            criteria=o3d.pipelines.registration.ICPConvergenceCriteria(max_iteration=30),
        )
        # With max_correspondence_dist=2.0 and clouds 141m apart,
        # O3D finds zero correspondences → fitness=0
        assert result.fitness == 0.0, (
            f"Expected zero fitness (no correspondences), got {result.fitness}"
        )
