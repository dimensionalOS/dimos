...
def test_teleop_suppresses_nav_and_cancels_goal(manager: MovementManager):
    ...
    # Nav is suppressed
    manager.cmd_vel.publish.reset_mock()
    manager._on_nav(_twist(lx=0.9))
    manager.cmd_vel.publish.assert_not_called()

    # stop_movement fired
    manager.stop_movement.publish.assert_called_once()

    # Goal cancelled with NaN
    cancel_msg = manager.goal.publish.call_args[0][0]
    assert math.isnan(cancel_msg.x)


def test_nav_resumes_after_cooldown(manager: MovementManager):
    ...
    manager._on_nav(_twist(lx=0.9))
    manager.cmd_vel.publish.assert_called_once()


def test_valid_click_publishes_goal(manager: MovementManager):
    """A valid click should publish to both goal and way_point."""
    click = _click(x=5.0, y=3.0, z=0.1)
    manager._on_click(click)
    manager.goal.publish.assert_called_once_with(click)
    manager.way_point.publish.assert_called_once_with(click)


def test_invalid_clicks_rejected(manager: MovementManager):
    ...
    manager.goal.publish.assert_not_called()
...
def test_tele_cmd_vel_scaling():
    ...
    published = module.cmd_vel.publish.call_args[0][0]
