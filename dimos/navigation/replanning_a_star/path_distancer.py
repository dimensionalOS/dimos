...
class PathDistancer:
    _path: NDArray[np.float64] | None = None
    _cumulative_dists: NDArray[np.float64] | None = None
    ...
    def find_lookahead_point(self, start_idx: int) -> NDArray[np.float64]:
        ...
        if segment_dist > 0:
            ...
            return self._path[idx] + t * (self._path[idx + 1] - self._path[idx])

        return self._path[idx]

    def distance_to_goal(self, current_pos: NDArray[np.float64]) -> float:
        return np.linalg.norm(self._path[-1] - current_pos)
