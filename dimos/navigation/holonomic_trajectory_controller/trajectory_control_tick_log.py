...
"""Per-control-tick telemetry record.
...
optional wall or simulation timestamps. Live runs pass ticks to
``JsonlTrajectoryControlTickSink`` when ``GlobalConfig.local_planner_trajectory_tick_log_path``
is set.

JSONL export (stable field names) lives in ``dimos.navigation.holonomic_trajectory_controller.trajectory_control_tick_export``
...
from dimos.navigation.holonomic_trajectory_controller.trajectory_metrics import (
