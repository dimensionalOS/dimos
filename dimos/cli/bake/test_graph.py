...
def module(module_id, inputs, outputs, threads=1):
    return ModuleInfo(
        id=module_id,
...
def test_matching_names_connect_and_classify():
...
def test_topics_carry_the_zenoh_namespace_and_the_message_type():
    graph = build_graph("host", [MAPPER, PLANNER])
    assert by_name(graph)["local_map"].topic == f"dimos/local_map/{PC2}"
    assert graph.topics()["mapper"]["lidar"] == f"dimos/lidar/{PC2}"


def test_pointcloud_topics_get_latest_wins_qos():
    graph = build_graph("host", [MAPPER, PLANNER])
...
def test_remap_connects_two_differently_named_ports():
...
def test_suppression_accepts_a_name_or_a_topic():
...
def test_suppressing_an_external_input_is_refused():
    with pytest.raises(BakeError, match="external input"):
        build_graph("host", [MAPPER, PLANNER], suppress=["lidar"])


def test_suppressing_an_unconsumed_output_warns():
...
def test_rendered_graph_is_grouped_by_direction():
    graph = build_graph("host", [MAPPER, PLANNER], suppress=["local_map"])
...
def test_graph_json_round_trips_the_classification():
    graph = build_graph("host", [MAPPER, PLANNER], suppress=["local_map"])
