...
- The module issues an opaque receipt with each panel-created plan. Execution atomically consumes that receipt and dispatches the exact captured plan, so another caller cannot replace the validated plan between the UI freshness check and execution.
