...
- [x] 2.5 Add adapter/runtime material needed to build the file-backed session and expose the receipt/context handoff while keeping unrelated `pi-spatial-agent-baseline` execution behavior unchanged.
...
- [x] 6.5 Add a gate integration test proving accepted submission plus normal scoring, complete matching receipt/hash, valid session, and successful pinned export promotes prediction/score, while missing/invalid evidence leaves no canonical result.

## 7. Narrow visualization-forbidden precursor

- [x] 7.1 Add exactly one narrowly scoped visualization-forbidden precursor command/mode that reuses the existing executor, adapter, submission, and normal scoring paths; require accepted submission, normal scoring, complete verified native session/receipt/hash, and successful pinned export.
