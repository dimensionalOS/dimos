...
- Add a narrow, one-run visualization-forbidden precursor check requiring an accepted submission, normal scoring (answer correctness is not required), a valid retained session, a matching receipt, and successful Pi CLI export.
...
- Leaves unrelated `pi-spatial-agent-baseline` execution behavior unchanged.
