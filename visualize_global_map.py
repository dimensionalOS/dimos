...
def _create_voxel_grid_from_pointcloud(pcd: o3d.geometry.PointCloud, voxel_size: float) -> o3d.geometry.VoxelGrid:
...
def on_global_map(msg: LidarMessage, _) -> None:
    ...
    if not map_lock:
        latest_map = msg
        print(f"Received global map: {len(msg.pointcloud.points)} points, "
              f"resolution={msg.resolution}")
