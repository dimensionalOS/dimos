...
| File | Holds |
|---|---|
| `src/wizards/<manufacturer>/<robot>.rs` | the whole wizard — one pure function per stage, one `observe` that reads the machine, the preflight, and the plan that lists the stages |
| `src/wizards/mod.rs` | one `Robot` arm: the variant, its `key`, and the `owned`/`ready`/`run` match arms |

Shared stages already exist: `setup/system_packages.rs` (system packages, uv),
`setup/system_config.rs` (sysctl, loopback multicast, memlock), `wizards/nvidia/jetson.rs`
(nvpmodel, jetson_clocks), `setup/verify.rs` (the critical checks). Compose those before writing
anything new.
...
A wizard never touches the machine. It reads probes, returns `Vec<Stage>`, and `run::run` does the
...
pub struct Probes {
    ...
    pub installed: Option<install_record::Installed>,
...
You never call this. Set `sudo: true` on the action; `run::run` wraps it and stops the stage with
...
- **A sudo `Run` carries its env in argv** (`["env", "K=V", "cmd", ...]`) — sudo's `env_reset` drops
  `Action.env`, and `run::sudo_env_violations` fails the run if you forget.
...
**2. `src/wizards/acme/demo.rs`** — the wizard itself: its stages, its `ready`, and its plan.
...
// ready        -> Robot::Demo => demo::ready(probes)
// run          -> Robot::Demo => demo::plan(args, probes, cfg, &installed)
```
...
In `src/wizards/<manufacturer>/<robot>.rs`:
...
Fixtures: build `Platform`/`Kernel`/`Tools`/`RcFile` literals (copy them from `wizards/mod.rs`'s
test module; a G1 fixture needs `ifaces` to hold its `--interface`). `install_record::TmpDir` is the
throwaway `HOME` when a test genuinely needs one — do not write a second one.
...
Then read `src/wizards/unitree/g1.rs` for the large case: an `observe` with six probes, a source build, a
