...
function that spawns a process, writes a file, or reads stdin. A stage with no actions means the
machine is already there, so a second run is free and `--dry-run` is trustworthy. Adding a robot:
