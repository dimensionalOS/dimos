...
export async function setupCpuFeature({ assumeSysDepsInstalled = false }: { assumeSysDepsInstalled?: boolean } = {}) {
    p.clearScreen()
    ...
    const res = await $$`pip install "dimos[cpu]"`
    if (res.code !== 0) {
    ...
    p.boringLog("- dimos[cpu] installed successfully")
}
