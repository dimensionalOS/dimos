...
export async function setupManipulationFeature({ assumeSysDepsInstalled = false }: { assumeSysDepsInstalled?: boolean } = {}) {
    p.clearScreen()
    p.header("Optional Feature: Manipulation")

    if (!assumeSysDepsInstalled) {
        console.log("- Likely system dependencies needed for manipulation (contact-graspnet, visualization, FCL, HDF5):")
    ...
    const res = await $$`pip install "dimos[manipulation]"`
    if (res.code !== 0) {
        p.error("pip install dimos[manipulation] failed. Please ensure system deps are installed and try again.")
        p.error("If issues persist, reinstall system deps or install the wheel from source with verbose logs.")
        return
    }
    p.boringLog("- dimos[manipulation] installed successfully")
}
