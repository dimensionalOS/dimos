    ...
    with requests.Session() as http, (ROOT / "logs/multiplayer-chromium.log").open("w") as log:
        ...
        try:
            ...
            if any(slot["occupied"] for slot in lobby["slots"]):
                raise RuntimeError("This checkpoint needs all six duck slots free.")
            ...
            for index in range(7):
                ...
                if index == 0:
                    ...
                    assert not evaluate(page, READ)["overflow"]
                    shot = page.command("Page.captureScreenshot", {"format": "png"})
                    (ROOT / "logs/football-club-lobby-mobile.png").write_bytes(
                        base64.b64decode(shot["data"])
                    )
                    page.command(
                        "Emulation.setDeviceMetricsOverride",
                ...
                if index < 6:
                    robot = ["duck2", "duck3", "duck1", "duck4", "duck5", "duck6"][index]
                    click(page, f"choose-{robot}")
                    click(page, "confirm-join")
                    ...
                    shot = page.command("Page.captureScreenshot", {"format": "png"})
                    (ROOT / "logs/football-club-world.png").write_bytes(
                        base64.b64decode(shot["data"])
                    )
                    evaluate(
                        page,
                    ...
                    print("seventh participant can observe; all six slots enforced", flush=True)
            first_state = observer.latest
            started = time.monotonic()
            time.sleep(10)
            last_state = observer.latest
            results["simulationTiming"] = {
                "wallSeconds": time.monotonic() - started,
                "before": first_state["simTime"],
                "after": last_state["simTime"],
            }
            assert (last_state["simTime"] - first_state["simTime"]) / (
                time.monotonic() - started
            ) > 0.8
            print("six-player simulation timing passed", flush=True)
            # Software WebGL across seven headless contexts can starve the browser's
            ...
            after = until(
                ...
                lambda s: (
                    s["state"] is not None
                    and abs(
                        s["state"]["poses"][roots["duck2"]][1]
                        - before["state"]["poses"][roots["duck2"]][1]
                    )
                    > 0.06
                ),
            )
            ...
            for id in ("duck1", "duck3", "duck4", "duck5", "duck6"):
            ...
            until(
                pages[1],
                lambda s: s["role"] == "observe" and len(s["choices"]) == 6,
            )
            evaluate(
                pages[1],
                """document.querySelectorAll('canvas').forEach(c =>
            ...
            click(pages[6], "choose-duck3")
            click(pages[6], "confirm-join")
            ...
            (ROOT / "logs/football-lobby-checkpoint.json").write_text(json.dumps(results, indent=2))
        finally:
            for page in pages:
                try:
                    evaluate(
